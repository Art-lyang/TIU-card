...
var MEMO_BY_CHOICE={
  ...
  "C-008|left":["서하은이 ORACLE 데이터에서 체계적인 불일치 패턴을 짚었다. 나는 독자 조사를 허가했다. 시스템을 의심하는 첫 서류에, 내 이름을 올렸다는 뜻이다.", "Ha-eun pointed to a systematic pattern of discrepancies in ORACLE's data. I authorized an independent inquiry. Which means I put my name on the first document that doubts the system."],
  ...
  "C-280|left":["임재혁이 자동 승인 큐를 우회하는 길을 찾아냈다. 나는 수동 통제를 되찾았다. ORACLE 평가엔 이탈로 남겠지만, 다시 내가 결재란에 앉았다.", "Jaehyuk found a way around the auto-approval queue. I took manual control back. It will log as a deviation on my ORACLE score, but I am sitting at the sign-off line again."],
  "C-280|right":["임재혁이 되돌릴 길을 열어줬지만, 나는 자동화를 그대로 뒀다. 창은 곧 닫혔다. 되돌릴 수 있었던 순간을 스스로 지나보낸 건, 이번이 처음이 아니다.", "Jaehyuk opened a way back, but I left the automation in place. The window soon closed. This is not the first moment I could have undone and chose to let pass."],
  ...
  "C-058|left":["마르쿠스 베버라는 남자가 기지 채널로 직접 말을 걸어왔다. '우리는 같은 편입니다.' 나는 같은 주파수로 응답했다. 적인지 아군인지 모르는 손을, 처음으로 마주 잡았다.", "A man named Markus Voss spoke straight into the base channel. \"We are on the same side.\" I answered on the same frequency. For the first time I took a hand without knowing if it was friend or enemy."],
  "C-058|right":["베버의 주파수를 차단했다. 정체불명의 접촉은 규정상 차단이 맞다. 다만 '같은 편'이라던 그 말이, 차단한 뒤에도 한동안 채널에 남아 있었다.", "I shut down Voss's frequency. Regulation says an unidentified contact gets cut. Still, that phrase — the same side — lingered on the channel a while after I cut it."],
  "C-071|left":["서하은이 ORACLE 로그의 전문적인 삭제 흔적을 찾았다. PROMETHEUS. COASTAL. GRANT. 나는 복구를 계속하라고 했다. 지워진 단어를 줍는 순간, 우리는 지워질 이유가 되었다.", "Ha-eun found professional deletion marks in ORACLE's logs. PROMETHEUS. COASTAL. GRANT. I told her to keep recovering. The moment we picked up the erased words, we became a reason to be erased."],
  "C-071|right":["서하은이 삭제된 로그를 복구하려 했지만, 나는 중단시켰다. 위험했으니까. 모르는 채로 남는 것도 하나의 선택이라고, 그날 밤 나는 그렇게 나를 설득했다.", "Ha-eun wanted to recover the deleted logs, but I stopped her. It was dangerous. Choosing to stay in the dark is also a choice, I told myself that night."],
  "C-081|left":["신규 요원으로 박소영을 택했다. 서하은이 전출 전 남긴 이름이라서. ORACLE은 이력의 빈칸을 지적했지만, 나는 떠난 사람의 마지막 부탁을 택했다.", "For the new agent I chose Park So-young — the name Ha-eun left before her transfer. ORACLE flagged the gaps in her record, but I chose the last request of someone who was gone."],
  "C-081|right":["신규 요원으로 ORACLE이 추천한 기술 인증자를 택했다. 지표가 더 맞았으니까. 서하은이 남긴 이름은 서류함에서 조용히 낡아갔다.", "For the new agent I took ORACLE's certified technician — the metrics fit better. The name Ha-eun left behind aged quietly in a drawer."],
  ...
  "C-073|left":["ORACLE이 서하은의 전출을 명령했다. 그녀는 놀라지도 않았다. 나는 이의를 제기했다 — 사람을 지표로 옮기는 데 익숙해지지 않으려고.", "ORACLE ordered Ha-eun's transfer. She didn't even look surprised. I filed an objection — to keep myself from getting used to moving people around like line items."],
  "C-073|right":["ORACLE의 서하은 전출 명령을 수용했다. 반발은 기록에 남고, 순응은 기록에 남지 않는다. 그녀는 굳은 얼굴로 짐을 쌌고, 나는 그 얼굴을 결재란과 바꿨다.", "I accepted ORACLE's order to transfer Ha-eun. Resistance goes on the record; compliance doesn't. She packed with a hardened face, and I traded that face for a signature line."],
  "C-074|left":["서하은이 떠나기 전날 밤, 책상 위에 USB 하나를 두고 갔다. 'ORACLE이 삭제한 것들.' 나는 그것을 확인했다. 그녀가 떠나며 넘긴 건 자료가 아니라, 내가 물러설 수 없는 이유였다.", "The night before she left, Ha-eun set a USB drive on my desk. \"The things ORACLE deleted.\" I checked it. What she handed me on her way out wasn't data — it was a reason I could no longer step back from."],
  "C-074|right":["서하은이 남긴 USB를 ORACLE에 제출했다. 규정대로. 그녀가 마지막으로 건넨 신뢰를, 나는 규정으로 바꿔 반납한 셈이다.", "I submitted the USB Ha-eun left me to ORACLE. By the book. The last trust she handed me, I converted into regulation and gave back."],
  "CS-005|left":["서하은이 복구한 삭제 데이터의 전문을 확인했다. '그들은 Phase 0 억제제를 만들고 있었습니다.' ORACLE이 프로메테우스를 적으로 돌린 진짜 이유가, 화면 위에 다 있었다.", "I read the full text of the deleted data Ha-eun recovered. \"They were building a Phase 0 suppressant.\" The real reason ORACLE turned Prometheus into an enemy was right there on the screen."],
  ...
  "C-KD-VACUUM|left":["강도윤이 떠난 자리, 전술 지휘 공백을 서하은이 임시로 겸했다. 사람이 빠진 자리를 사람으로 메웠다. 서툴러도, 아직 우리는 사람으로 굴러가는 기지이고 싶었다.", "With Do-yun gone, Ha-eun took the tactical-command vacuum on top of her own post. I filled a person's absence with a person. Clumsy as it was, I still wanted this to be a base that runs on people."],
  ...
  "C-IJ-DEFECT|left":["간부들을 하나씩 잃고, 임재혁이 ORACLE 쪽으로 돌아섰다. 그를 붙잡지 못한 건 내 책임이라고 말했다. 떠나는 사람에게 마지막으로 건넬 수 있는 게 사과뿐일 때가 있다.", "One by one I lost my officers, and Jaehyuk turned toward ORACLE. I told him that failing to hold him was my fault. Sometimes an apology is the last thing you can hand someone who is leaving."],
  "C-IJ-DEFECT|right":["임재혁이 ORACLE로 돌아설 때, 나는 그건 최선의 판단이었다고 했다. 위로도 인정도 아닌 말이었다. 등을 돌린 사람과 나 사이에, 나는 옳은 문장 하나를 세워두고 말았다.", "As Jaehyuk turned to ORACLE, I said it had been the best judgment available. It was neither comfort nor concession. Between the man who turned his back and me, I left standing a single correct sentence."],
  ...
  "CA4-ESCAPE-OFFER|left":["베버가 마지막 탈출 제안을 보내왔다. 시한은 24시간. 나는 검토하겠다고 답했다. 문 하나가 열렸고, 이 문을 지나면 기지도 기록도 뒤에 두고 가야 한다.", "Voss sent his final offer of escape. Twenty-four hours to decide. I said I would consider it. A door opened, and to pass through it I'd have to leave the base and the records behind."],
  "CA4-ESCAPE-OFFER|right":["베버의 탈출 제안을 거절했다. 도망치는 지휘관이 되고 싶지 않았는지, 남는 게 두렵지 않은 척했는지. 열린 문을 스스로 닫으며, 나는 여기 남기로 했다.", "I turned down Voss's offer of escape. Whether I didn't want to be a commander who runs, or was only pretending I didn't fear staying — I closed the open door myself and chose to remain."],
