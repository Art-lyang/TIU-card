...
function EndingScreen(p){
  var all=[{id:'A',name:'완벽한 도구',hint:'ORACLE의 최고 신임을 얻으라'},{id:'B',name:'각성',hint:'진실의 단편을 목격하라'},{id:'C_cs',name:'봉쇄 성공',hint:'완벽한 봉쇄를 달성하라'},{id:'C_cst',name:'자충수',hint:'봉쇄의 대가를 치르다'},{id:'C_c',name:'봉쇄 붕괴',hint:'봉쇄선이 무너지다'},{id:'C_r',name:'자원 고갈',hint:'기지가 기능을 잃다'},{id:'C_t',name:'신뢰 상실',hint:'동료들이 떠나다'},{id:'C_o',name:'접속 차단',hint:'ORACLE이 당신을 버리다'},{id:'D',name:'조용한 자유',hint:'반란 속의 해방'},{id:'F',name:'[데이터 손상]',hint:'???'}];
...
function FieldMission(p){
  ...
  return h('div',{className:'screen',style:{overflow:'hidden'}},
    ...
    showChoices&&h('div',{style:{width:'100%',maxWidth:420,flexShrink:0,display:'flex',flexDirection:'column',gap:6,padding:'6px 0'}},
      node.choices.map(function(c,i){return h('button',{key:i,onClick:function(){handleChoice(c)},style:{background:'var(--ui-bg)',border:'1px solid var(--ui-border)',borderRadius:4,padding:'10px 14px',cursor:'pointer',textAlign:'left',color:c.next==='end'?'#f0a030':'var(--ui)',fontSize:12,lineHeight:1.5,fontFamily:'inherit'}},h('span',{style:{marginRight:6,opacity:0.5}},'▸'),c.label)})
    ),
