...
var ENDING_CATEGORY_META = {
  compliance:  { label: 'ORACLE 복종', color: '#4ae' },
  resistance:  { label: '저항',        color: '#f0a030' },
  containment: { label: '봉쇄',        color: '#c72' },
  escape:      { label: '탈출',        color: '#7c9' },
  observer:    { label: 'Observer',    color: '#c5c' },
  neutral:     { label: '중립',        color: '#888' },
  failure:     { label: '실패',        color: '#a44' },
  uprising:    { label: '점거',        color: '#d80' }
};
...
function EndingScreen(p) {
      ...
      h('div', { style: { width: '100%', maxWidth: 500, padding: '20px 0', flex: 1, overflowY: 'auto' } },
        ...
        narr && h('div', {
          ...
          return h('div', { key: i, style: {
            color: isCmd ? '#f0a030' : 'var(--ui)',
            fontFamily: isCmd ? "'Share Tech Mono',monospace" : 'inherit',
            fontWeight: isCmd ? 'bold' : 'normal',
            minHeight: isEmpty ? 10 : 'auto'
          } }, isEmpty ? '\u00A0' : l);
        })),
  ...
  // ───── 갤러리 뷰 ─────
  // 카테고리별 그룹화 (해금된 엔딩만 이미지, 미해금은 실루엣)
  var rows = [];
  ENDING_CATALOG.forEach(function(e) {
    ...
    rows.push(h('div', {
      key: e.id,
      onClick: locked ? null : function(){ setSel(e); },
      style: {
        position: 'relative',
        background: locked ? '#080808' : 'var(--ui-bg)',
        border: '1px solid ' + (locked ? '#111' : meta.color),
        borderRadius: 4,
        overflow: 'hidden',
      ...
      h('div', {
        style: {
          height: 120,
          background: img ? ('url(' + img + ') center/cover no-repeat') : '#050505',
          position: 'relative',
        ...
        locked && h('div', {
          style: {
            ...
            fontFamily: "'Share Tech Mono',monospace", fontSize: 28, color: '#222', letterSpacing: 4
          }
        }, '?'),
        !locked && h('div', {
          style: {
            ...
            color: meta.color, background: 'rgba(0,0,0,.5)', padding: '2px 6px', borderRadius: 2
          }
        }, '[' + e.id + ']')
      ),
      // 본문
  ...
  return h('div', { className: 'screen' },
    h('div', { style: { width: '100%', maxWidth: 500, padding: '20px 12px', flex: 1, overflowY: 'auto' } },
      h('div', {
        style: { fontFamily: "'Share Tech Mono',monospace", fontSize: 11, color: 'var(--ui-dim)', letterSpacing: 2, textAlign: 'center', marginBottom: 6 }
      }, 'SESSION ARCHIVE — ENDING GALLERY'),
      h('div', {
        style: { fontSize: 12, color: '#888', textAlign: 'center', marginBottom: 18 }
      }, '총 세션: ' + (p.sessions || 0) + '  |  엔딩 해금: ' + unlockedCount + ' / ' + total),
