...
function EvidenceTable(p) {
  ...
  if (!show && !p.forceOpen) return h('div', { style: { margin: '12px auto 0', maxWidth: 440,
    ...
    h('div', { onClick: function() { setShow(true) }, style: { cursor: 'pointer',
      ...
      h('span', { style: { fontSize: 9, color: 'rgba(var(--ui-rgb),.4)' } }, '\u25BC OPEN')),
  ...
  return h('div', { style: { margin: '12px auto 0', maxWidth: 440, border: '1px solid rgba(var(--ui-rgb),.12)',
    ...
    h('div', { onClick: function() { setShow(false) },
      ...
      h('span', { style: { fontSize: 9, color: 'rgba(var(--ui-rgb),.4)' } }, '\u25B2 CLOSE')),
