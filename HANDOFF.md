...
## 0-0. 2026-05-18 현재 기준 — Issue #33 P1~P3 출시 전 밸런스/상태분기 보완

### 이번 세션 완료

- P1: `CR-005`, `CE-031`, `CA-SEED-04`의 무효 선택지/체감 없는 선택지를 제거하고, 물자·신뢰·평가·GI 사이의 명확한 트레이드오프로 조정.
- P1: `CE-014`의 저항 선택을 `t` 단독 완충이 아닌 봉쇄/자원/신뢰/평가 축으로 재분배하고, 순응 선택의 GI 상승을 `+4`에서 `+3`으로 완충.
- P1: `CE-026`과 Act 4 저항/탈출/위기 카드에서 강도윤 중상(`LOG-074-DONE`) 상태를 직접 파견/현장 동행 불가 상태로 반영.
- P1: 보상 단계 광역 소프트 플로어와 1회성 `resourceReserve` 구제를 제거. 일반 선택/보상은 자원을 0까지 떨어뜨릴 수 있고, `CE-005`, `CE-042/B`만 좁은 예외로 유지.
- P1: `tools/simulator_v3.py`의 레거시 광역 바닥 보정을 런타임 규칙과 동기화하고, 카드별 `floor` 예외만 반영하도록 조정.
- P2: `C-253` 폭로 창을 `day>=32`에서 `day>=30`으로 완화.
- P2: SPEC-015 직접 잠수와 M-001 강도윤 합동 돌입 선택지가 경상/중상/사망 상태를 모두 차단하도록 `blockLogs` 적용.
- P2: `tools/issue22_audit.js`가 `ending_C_cst` 공유 아트 주석 대신 전용 이미지 매핑을 검사하도록 갱신하고, `lang-cards-flow-en.js` 로드를 추가.
- P3: `tools/critical-audit.js`에 Issue #33 회귀 가드 추가. CE-026 중상 차단, 무효 선택지, C-253 창, `ending_C_cst` 전용 매핑, 자원 0 보정 회귀를 감시.
- P3: 상태 태그에서 강도윤 `LOG-074-DONE` 라벨을 “부상”이 아닌 “중상 / Critical Wound”로 표시.
...
1. 광역 보정 제거로 자동 시뮬레이터 난도가 크게 올라갔다. 출시 밸런스는 인간 플레이에서 “실수 누적은 처벌하되 불합리하게 빠른 즉사로 느껴지지 않는지”를 재측정해야 한다.
2. `data-cards-resist-hint.js` 계열은 전역이 아니라 해당 팩 안에서 `t` 보상 비중이 높다. 이번 패치에서는 명시 결함 카드만 조정했고, 다음 밸런스 패스에서 RH/HH/CB 카드군의 `t` 편향을 별도 조정 후보로 본다.
3. `LOG-074-DONE`은 이브닝 챗에서는 중상 전용 대사가 우선된다. 현장 파견 차단은 적용했지만, 단순 대사 카드의 중상 호환 문장 품질은 추가 감수 여지가 있다.
