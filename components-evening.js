...
function EveningChat2(p){
  ...
  if(selChar&&!chat){
    ...
    if(eventMatches.length>0){chat=eventMatches[0]}
      ...
      // Fallback 2: unused only, full day-range + dayCap guard (H-E1/E6 — keep dayMax & dayCap consistent so expired lines never resurface)
      matches=EVENING_CHATS.filter(function(ec){return ec.char===selChar.name&&ec.act.indexOf(p.act)>=0&&p.day>=ec.dayMin&&p.day<=ec.dayMax&&ec.dayMin<=dayCap&&!ecUsed(ec)&&skipIntro(ec)&&evalCond(ec)&&deckCond(ec)}).sort(sortByDay);
      if(matches.length>0){chat=matches[0]}
      else{
        // Fallback 3: allow reused chats, but still enforce dayMax + dayCap (H-E6 — low-trust must not see late-game content)
        matches=EVENING_CHATS.filter(function(ec){return ec.char===selChar.name&&ec.act.indexOf(p.act)>=0&&p.day>=ec.dayMin&&p.day<=ec.dayMax&&ec.dayMin<=dayCap&&skipIntro(ec)&&evalCond(ec)&&deckCond(ec)}).sort(sortByDay);
        if(matches.length>0)chat=matches[matches.length-1];
        else{
          // Fallback 4: still enforces dayMax + dayCap (H-E1/E6)
          matches=EVENING_CHATS.filter(function(ec){return ec.char===selChar.name&&ec.act.indexOf(p.act)>=0&&p.day>=ec.dayMin&&p.day<=ec.dayMax&&ec.dayMin<=dayCap&&skipIntro(ec)&&evalCond(ec)&&deckCond(ec)}).sort(sortByDay);
  ...
  // S2: resume evening chat at saved line index when re-entering the same chat.
  useEffect(function(){
    if(!selChar)return;
    var startLi=0;
    try{
      var ck=chat?ecKey(chat):null;
      var saved=(typeof Save!=='undefined')?Save.get('ts_eveningLineState',null):null;
      if(saved&&ck&&saved.chatKey===ck&&saved.charName===selChar.name&&saved.day===p.day&&saved.act===p.act){
        var lineCount=Array.isArray(chatLines)?chatLines.length:0;
        startLi=Math.min(Math.max(0,saved.li||0),Math.max(0,lineCount-1));
      }
    }catch(e){}
    setLi(startLi);setCi(0);setDone(false);setChoiceDone(false);setReplyLine('');
  },[selChar,chat]);
  // S2: persist current line index so reload mid-chat resumes here.
  useEffect(function(){
    if(!selChar||!chat||done||choiceDone)return;
    try{
      if(typeof Save!=='undefined')Save.set('ts_eveningLineState',{chatKey:ecKey(chat),charName:selChar.name,day:p.day,act:p.act,li:li});
    }catch(e){}
  },[li,chat,selChar,done,choiceDone]);
  ...
  var pickChar=function(c){
    ...
    if(!chosen){
      // Fallback 2 (H-E1/E6): keep dayMax + dayCap2 — expired or above-tier lines must not surface
      m2=EVENING_CHATS.filter(function(ec){return ec.char===c.name&&ec.act.indexOf(p.act)>=0&&p.day>=ec.dayMin&&p.day<=ec.dayMax&&ec.dayMin<=dayCap2&&!ecUsed(ec)&&si2(ec)&&ec2(ec)&&dc2(ec)}).sort(sortD);
      if(m2.length>0)chosen=m2[0];
      else{
        // Fallback 3 (H-E6): allow reused, still enforce dayCap2
        m2=EVENING_CHATS.filter(function(ec){return ec.char===c.name&&ec.act.indexOf(p.act)>=0&&p.day>=ec.dayMin&&p.day<=ec.dayMax&&ec.dayMin<=dayCap2&&si2(ec)&&ec2(ec)&&dc2(ec)}).sort(sortD);
        if(m2.length>0)chosen=m2[m2.length-1];
        else{
          // Fallback 4 (H-E1/E6): final fallback also keeps both guards
          m2=EVENING_CHATS.filter(function(ec){return ec.char===c.name&&ec.act.indexOf(p.act)>=0&&p.day>=ec.dayMin&&p.day<=ec.dayMax&&ec.dayMin<=dayCap2&&si2(ec)&&ec2(ec)&&dc2(ec)}).sort(sortD);
  ...
  var returnToEvening=function(){
    ...
    // S2: clear resume state once the chat is closed.
    try{if(typeof Save!=='undefined')Save.del('ts_eveningLineState')}catch(e){}
