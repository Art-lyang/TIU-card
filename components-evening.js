...
function EveningChat(p){
  ...
  if(!selChar)return h(React.Fragment,null,
    h('div',{className:'screen'},
      ...
      (p.logs&&p.logs.indexOf('LOG-EV-UNLOCK')>=0)&&h('button',{className:'btn evening-evidence-open',onClick:function(){if(p.onOpenEvidence)p.onOpenEvidence()},style:{display:'block',margin:'16px auto 0',fontSize:12,padding:'10px 24px',letterSpacing:1.5}},tt('evening.openEvidence',null,'▸ 조사 테이블 — 증거 조합')),
...
function EveningChat2(p){
  ...
  // 이브닝 챗 라인 블립 제거 — 실기기 테스트에서 매 라인 반복음이 소음으로 확인됨 (BUILD 426)
  ...
  var portrait=CHAR_IMG[selChar.name]||null;
  var _emoPortrait=(typeof resolveEveningEmotionImg==='function')?resolveEveningEmotionImg(selChar.key,{tier:(typeof getTrustTier==='function')?getTrustTier(p.trust,selChar.key):'mid',act:p.act,chatKey:(chat&&typeof getChatI18nKey==='function')?getChatI18nKey(chat):'',logs:p.logs}):null;
