...
function EveningChat(p){
  ...
  var pickChar=function(c){
    ...
    if(m2.length===0)m2=EVENING_CHATS.filter(function(ec){return ec.char===c.name&&ec.act.indexOf(p.act)>=0&&p.day>=ec.dayMin&&p.day<=ec.dayMax&&ec.dayMin<=dayCap2&&!ecUsed(ec)&&si2(ec)&&ec2(ec)&&dc2(ec)}).sort(sortD);
  };
  ...
  if(!selChar)return h(React.Fragment,null,
    h('div',{className:'screen'},
      ...
      !showSkipConfirm&&h('button',{className:'btn',style:{display:'block',order:20,margin:'20px auto 0',fontSize:11,padding:'8px 20px',opacity:0.5},onClick:function(){setShowSkipConfirm(true)}},'[ '+tt('evening.skip',null,'건너뛰기')+' ]')),
    ...
    showSkipConfirm&&h('div',{style:{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',zIndex:9999,maxWidth:320,width:'90%',boxSizing:'border-box',border:'1px solid rgba(var(--ui-rgb),.25)',background:'rgba(3,7,8,.98)',borderRadius:4,padding:'18px 20px',textAlign:'center',boxShadow:'0 4px 24px rgba(0,0,0,.6)'}},
