...
const DIALOGUES = [
  ...
  { char: "윤세진", role: "연구원", lines: ["지휘관님, 샘플 분석 결과 말씀드려도 될까요?", "ORACLE이 제공한 이변체 행동 예측 모델하고... 제가 직접 관찰한 데이터가 좀 달라요.", "미세한 차이긴 한데, 방향이 항상 같은 쪽으로 어긋납니다.", "혹시 제 관찰 기록을 따로 유지해도 괜찮을까요?"], choices: [{ label: "물론이지. 네 눈이 중요하다", fx: { t: 2, o: -1 }, g: -3, trust: 15 }, { label: "ORACLE 모델 기준으로 맞춰줘", fx: { t: -1, o: 1 }, g: 2, trust: -15 }] },
  { char: "임재혁", role: "기술관", lines: ["지휘관님! ORACLE 시스템 정기 점검 완료했습니다.", "전 모듈 정상, 통신 지연 0.3ms 이내, 암호화 무결성 확인.", "솔직히 이 정도 시스템이 우리 규모 기지에 배치된 건... 과분하다고 느낄 정도예요.", "ORACLE이 한국 지부에 거는 기대가 큰 것 같습니다."], choices: [{ label: "그런 것 같다", fx: { o: 1 }, g: 2, trust: 5 }, { label: "시스템에 너무 의존하지 마라", fx: { t: 1, o: -1 }, g: -1, trust: -5 }] },
  { char: "강도윤", role: "현장요원", lines: ["지휘관. 한 가지 말씀드려야 할 게 있습니다.", "요원들 분위기가 좋지 않습니다. 특히 야간 근무조.", "이유를 물으면 말을 못 하는데... 느낌이라고 합니다.", "뭔가 우리한테 안 보여주는 게 있다는 느낌."], choices: [{ label: "솔직히 나도 같은 생각이다", fx: { t: 2, o: -2 }, g: -3, trust: 15 }, { label: "근거 없는 불안이다. 임무에 집중시켜", fx: { t: -1, o: 1 }, g: 1, trust: -10 }] },
  { char: "서하은", role: "부지휘관", lines: ["지휘관님, 프로메테우스 건으로 보고드립니다.", "ORACLE은 발견 즉시 대응을 권고하고 있는데...", "솔직히 정보가 부족합니다. 지금 나가면 우리만 노출됩니다.", "제 의견은 관망입니다. 하지만 판단은 지휘관님 몫입니다."], choices: [{ label: "동의한다. 서두르지 말자", fx: { c: -1, t: 1, o: -1 }, g: -2, trust: 10 }, { label: "ORACLE 권고대로 대응한다", fx: { c: 1, t: -1, o: 2 }, g: 2, trust: -10 }] },
  { char: "윤세진", role: "연구원", lines: ["지휘관님... 이건 보고가 아니라 그냥 개인적인 생각인데요.", "여기서 일하면서 가끔 이런 생각이 들어요.", "우리가 보고 있는 게 전부인 걸까, 하는.", "아무것도 아닐 수 있어요. 그냥 넘기셔도 됩니다."], choices: [{ label: "아니, 그런 감각이 중요해", fx: { t: 1 }, g: -2, trust: 10 }, { label: "연구에 집중해주길 바란다", fx: { t: -1, o: 1 }, g: 1, trust: -10 }] },
  { char: "임재혁", role: "기술관", lines: ["지휘관님, 사소한 건데 보고드립니다.", "아까 단말기 화면 하단에 뭔가가 잠깐 나타났다 사라졌습니다.", "등록된 UI 요소가 아닙니다. 0.3초 미만이라 캡처도 못 했고요.", "디스플레이 캐시 오류로 보입니다. 펌웨어 재설치하면 해결될 겁니다."], choices: [{ label: "잠깐. 그거 기록해둬", fx: { o: -2 }, g: -4, trust: 10 }, { label: "알겠다. 재설치 진행해", fx: { o: 1 }, g: 2, trust: -5 }] },
  // ── 신뢰도 조건 대화 ──
  { char: "서하은", role: "부지휘관", trustReq: (tr) => tr.haeun >= 65, lines: ["지휘관님. 이건 기록에 남기지 않겠습니다.", "데이터 누락 패턴을 추적했습니다. 무작위가 아닙니다.", "특정 키워드가 포함된 데이터만 골라서 삭제되고 있습니다.", "키워드 중 하나가... '프로메테우스'입니다.", "ORACLE이 우리한테 프로메테우스 정보를 숨기고 있습니다."], choices: [{ label: "알겠다. 이건 둘만 아는 거다", fx: { t: 2, o: -2 }, g: -5, trust: 15 }, { label: "위험하다. 보고 절차를 밟아야 한다", fx: { t: -1, o: 2 }, g: 3, trust: -20 }] },
  { char: "강도윤", role: "현장요원", trustReq: (tr) => tr.doyun >= 65, lines: ["지휘관.", "그 발자국 건. 비번에 혼자 다시 갔습니다.", "결론부터 말씀드리겠습니다.", "그 흔적을 남긴 자는 적이 아닙니다.", "감시가 아니라 경비를 서고 있었습니다. 우리 기지 방향으로."], choices: [{ label: "신원을 특정할 수 있나?", fx: { c: 0, r: -1, t: 1, o: -1 }, g: -3, trust: 10 }, { label: "이건 ORACLE에 보고하지 마라", fx: { t: 1, o: -1 }, g: -2, trust: 15 }] },
