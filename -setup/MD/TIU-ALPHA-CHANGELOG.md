...
> 최신 스냅샷: **2026-05-06** (BUILD_VER=160)
> 이전 스냅샷: 2026-04-24 (BUILD_VER=60) / 2026-04-23 (BUILD_VER=54) / 2026-04-20 (2차, BUILD_VER=22) / 2026-04-20 (1차, BUILD_VER=21) / 2026-04-19
> 브랜치: `main` (origin/main 동기화 완료)
> 대상 빌드: **TIU late beta → release-candidate 준비**
> 대응 GDD: `-setup/GDD/TIU-GAME-GDD-v10.md`

---

## 2026-05-06 스냅샷 (BUILD_VER 60 → 160) — 무결성/이브닝챗/조사테이블/세이브 안정화 + DLC 허브 제외

### 핵심 QA 결과

| 항목 | 결과 |
|---|---|
| 정적 validator | 파일 로드 75/실패 0, 이슈 0건 |
| 카드 | 521장 / 고유 ID 521 |
| 체인 | 메인 18 + 사건/후속 10 |
| 미션 | 15개 |
| 미니게임 연동 | 9개 임무 참조 정상 |
...
- **LOG/카드 무결성 정리**: LOG ID 충돌, 카드 act 누락, 고아 카드, 생산되지 않는 LOG 참조, 엔딩 필수 LOG 미생산 항목을 정리하고 validator 0건 통과 기준으로 갱신.
- **한국어 이브닝챗 누출 수정**: 한국어 플레이 중 일부 이브닝챗이 영어 content bucket을 선택하던 문제를 locale 우선 해석으로 교정.
- **조사테이블 해금 안내 교정**: `LOG-EV-UNLOCK` 전에는 "조사테이블 확인 후 다음 단계" 안내가 출력되지 않도록 수정. Act 2 임재혁 이브닝챗을 통한 해금 루트를 기준화.
- **3슬롯 세이브/로드 QA**: 슬롯 1~3 독립 저장/로드, logs/usedEvening/facilities/evidence combos/activeSpecs/chainQueue 보존 확인. 불가능한 DAY/ACT 조합은 정상화.
- **현장임무 연동 확인**: 현장임무 후속 카드 생성, 미니게임 참조, 미션 노드 참조, 중복/스택 누적 위험을 validator와 브라우저 QA로 재확인.
- **UI 원형 보존 기준 복구**: 요청되지 않은 버튼/게이지/선택지 숫자 표기 변경을 되돌리고, `gauge_fill.png` 계열 게이지와 스와이프 홀딩 자원 프리뷰, 하단 화살표 선택 표시를 기준으로 문서화.
- **카드 분배/난도 보강**: Act 2 정보 과밀 일부를 Act 3/4로 분산하고, Act 4가 무조건 자원압박만 주는 흐름이 되지 않도록 보상/뉴스/완충 카드 방향을 보강.
- **DLC 허브 제외**: 현재 본편 관리 범위를 줄이기 위해 ScenarioHub/DLC 선택 창과 허브 탭 이미지를 런타임에서 제거. `새 세션 시작`은 본편 새 세션으로 직접 진입하며 DLC는 추후 별도 작업으로 분리.

### 남은 출시 준비

- 한국어 완주 QA: Act 1~4, 엔딩 루트, 세이브/로드 분기, 이브닝챗/조사테이블 재점검.
- 영어 인간 감수: UI는 상당히 안정화되었으나 장문 카드, 이브닝챗, 조사테이블 로그는 최종 톤 감수 필요.
- 마케팅 자산: Steam/itch 스크린샷 5~7장, GIF 2~3개, 60초 트레일러, 캡슐 그래픽.
- 커뮤니티 준비: 베타 피드백 수집용 Discord/폼/이슈 템플릿 준비 권장.
