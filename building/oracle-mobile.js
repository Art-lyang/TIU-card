...
function renderMobileGrid(floorId) {
  ...
  if (floor.masked) {
    ...
    mask.innerHTML =
      ...
      '<div class="lock-text blink">' + htmlText(ui("accessDenied")) + '</div>' +
      '<div class="lock-sub">' + htmlText(ui("clearanceReq")) + '</div>';
...
function renderMobileRoomList(floor, grid, dimmed) {
  ...
  floor.rooms.forEach(function(room) {
    ...
    var upHtml = room.upgradable
      ? '<div class="mr-upgrade">' + htmlText(ui("upgradeAvail")) + '</div>' : "";

    var artSvg = (typeof ROOM_ART !== 'undefined') ? ROOM_ART.get(room.type) : '';
    el.innerHTML =
      (artSvg ? '<div class="mr-art">' + artSvg + '</div>' : '') +
      '<div class="mr-info-bar">' +
        '<div class="mr-name" style="color:' + tc.label + '">' + htmlText(t(room.name)) + '</div>' +
        '<div class="mr-type-row"><span class="mr-type">' + htmlText(tType(room.type)) + '</span>' +
...
function openMobileSheet(room) {
  ...
  var upHtml = room.upgradable
    ? '<div style="margin-top:10px;padding:5px 8px;border:1px solid #1a3a1a;' +
      ...
      'display:inline-block">' + htmlText(ui("upgradeAvail")) + '</div>'
    : "";

  body.innerHTML =
    '<div class="room-title" style="color:' + tc.label + '">' + htmlText(t(room.name)) + '</div>' +
    '<div class="room-type">' + htmlText(tType(room.type)) + '</div>' +
    '<div class="room-desc">' + htmlText(t(room.desc)) + '</div>' +
    '<div class="room-status' + statusClass + '">' + htmlText(statusText) + '</div>' +
    upHtml;
