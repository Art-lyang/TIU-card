...
function Tutorial(p){
  ...
  return h('div',{className:'screen'},h('div',{style:{width:'100%',maxWidth:420,padding:'24px 0',flex:1,display:'flex',flexDirection:'column'}},h('div',{style:{fontFamily:"'Share Tech Mono',monospace",fontSize:11,color:'#1a6a1a',textAlign:'center',marginBottom:12,letterSpacing:2}},'ORACLE BRIEFING — '+(step+1)+'/'+steps.length),h('div',{style:{flex:1,display:'flex',flexDirection:'column',justifyContent:'center',overflowY:'auto',minHeight:0}},st.lines.slice(0,shown).map(function(l,i){var txt=String(l||' ');var isHeader=txt.indexOf('[')===0;var isIcon='◆◇○●'.split('').some(function(c){return txt.indexOf(c)>=0});var isList=txt.indexOf('▸')===0;var isCtrl=txt.indexOf('←')>=0||txt.indexOf('→')>=0;var col=isHeader?'#f0a030':isIcon?'#f0a030':isList?'#9dff74':isCtrl?'#33ff33':'#ccddcc';var sz=isHeader?12:isIcon?13:14;var al=(isList)?'left':'center';return h('div',{key:step+'-'+i,style:{fontSize:sz,lineHeight:1.7,color:col,textAlign:al,fontFamily:isHeader?"'Share Tech Mono',monospace":'inherit',letterSpacing:isHeader?1:0,paddingLeft:isList?16:0,animation:'fadeIn 0.3s ease'}},txt)})),bv&&h('div',{style:{display:'flex',flexDirection:'column',gap:10,alignItems:'center',paddingBottom:20,flexShrink:0}},st.choices.map(function(c,i){return h('button',{key:i,className:'btn'+(i===0?' btn-amber':''),onClick:function(){if(c.next===-1)p.onDone();else setStep(c.next)}},c.label)}))));
}
