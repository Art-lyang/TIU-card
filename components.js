...
function Stats(p){
  var sm=[{k:'c',l:'봉쇄',ic:'◆'},{k:'r',l:'자원',ic:'◇'},{k:'t',l:'신뢰',ic:'○'},{k:'o',l:'평가',ic:'●'}];
  return h('div',{className:'stats'},h('div',{className:'stats-hdr'},'ORACLE STATUS — DAY '+p.stats.day),sm.map(function(s){var v=p.stats[s.k],d=v<=15,hi=v>=85,c=d?'#ff4444':hi&&s.k==='o'?'#f0a030':'#33ff33';var rc=d?'stat-row stat-danger':hi?'stat-row stat-high':'stat-row';return h('div',{key:s.k,className:rc},h('span',{className:'stat-label',style:{color:c}},s.ic+' '+s.l),h('div',{className:'bar-bg'},h('div',{className:'bar-fill',style:{width:v+'%',background:c,boxShadow:'0 0 '+(d?'8':'4')+'px '+c}})),h('span',{className:'stat-val',style:{color:d?'#ff4444':'#33ff33'}},v))}));
}
function CardC(p){
  var card=p.card,gi=p.gi||0;
  var s1=useState(0),dx=s1[0],setDx=s1[1];var s2=useState(false),dragging=s2[0],setDragging=s2[1];var s3=useState(0),sx=s3[0],setSx=s3[1];var s4=useState(null),chosen=s4[0],setChosen=s4[1];var s5=useState(false),isGlitch=s5[0],setIsGlitch=s5[1];
  useEffect(function(){setIsGlitch(gi<=30&&(p.day||1)>=5&&Math.random()>0.7)},[card]);
  ...
  var pc=card.priority==='상'?'#ff4444':card.priority==='중'?'#f0a030':'#33ff33';
  var pcClass=card.priority==='상'?' card-p-high':card.priority==='중'?' card-p-mid':'';
  ...
  return h('div',{className:'card-area'},
    h('div',{className:'hint hint-l',style:{opacity:dx<-30?Math.min(1,Math.abs(dx)/th):0}},'← '+card.left.label),
    h('div',{className:'hint hint-r',style:{opacity:dx>30?Math.min(1,dx/th):0}},card.right.label+' →'),
    h('div',{className:'card'+pcClass+(isGlitch?' screen-glitch':''),style:{transform:'translateX('+tx+'px) rotate('+(tx*0.04)+'deg)',transition:dragging?'none':'transform 0.3s ease',opacity:chosen?0:1},
      ...
      h('div',{className:'card-hdr'+(isGlitch?' glitch-active':'')},h('span',null,'ORACLE 통신'),h('span',{style:{color:pc}},'우선순위: '+card.priority)),
      h('div',{className:'card-body'+(isGlitch?' glitch-active':'')},card.msg),
      h('div',{className:'card-ft'},h('span',null,'← '+card.left.label),h('span',null,card.right.label+' →'))
    ));
}
function News(p){
  var s=useState(0),shown=s[0],setShown=s[1];var bg=Math.random()>0.5?IMG.bg_seoul_a:IMG.bg_seoul_b;
  useEffect(function(){if(shown<p.headlines.length){var t=setTimeout(function(){setShown(function(v){return v+1})},600);return function(){clearTimeout(t)}}},[shown,p.headlines.length]);
  return h('div',{className:'news',style:{position:'relative'}},bg&&h('div',{className:'bg-overlay',style:{backgroundImage:'url('+bg+')',opacity:0.15,height:120,left:0,right:0,width:'100%'}}),h('div',{className:'news-hdr'},'ORACLE 정보 수신'),h('div',{style:{color:'#1a3a1a',textAlign:'center',marginBottom:20}},'━━━━━━━━━━━━━━━━━━━━━━━━━'),p.headlines.slice(0,shown).map(function(l,i){var s=String(l||'');return h('div',{key:i,className:'news-line'+(s.indexOf('분류 오류')>=0?' news-glitch':'')+(s.indexOf('삭제됨')>=0?' news-deleted':'')},s)}),shown>=p.headlines.length&&h('button',{className:'btn',style:{display:'block',margin:'24px auto 0'},onClick:p.onContinue},'[ 다음 날 ]'));
}
function GameOver(p){
  var msg=p.gi>50?"요원의 헌신적 복무에 감사드립니다.":p.gi>25?"세션이 종료됩니다. 결과가 기록되었습니다.":"비표준 운영 패턴 감지. 세션 데이터 분석 중...";
  return h('div',{className:'boot'},h('div',{style:{fontSize:13,lineHeight:1.9,maxWidth:420,width:'100%',textAlign:'center'}},h('div',{className:'go-title'},'─── SESSION TERMINATED ───'),h('div',{className:'go-reason'},p.reason),h('div',{className:'go-section'},'── ORACLE 최종 보고 ──'),h('div',{className:'go-stat'},'운영 기간: '+p.stats.day+'일'),h('div',{className:'go-stat'},'봉쇄: '+p.stats.c+' | 자원: '+p.stats.r+' | 신뢰: '+p.stats.t+' | 평가: '+p.stats.o),h('div',{className:'go-msg'},'"'+msg+'"')),h('button',{className:'btn btn-amber',onClick:p.onRestart},'[ 세션 재개시 ]'),h('div',{style:{display:'flex',gap:10,marginTop:16}},h('button',{className:'btn',style:{fontSize:12,padding:'10px 18px',minHeight:44,marginTop:0},onClick:p.onLogs},'기록'),h('button',{className:'btn',style:{fontSize:12,padding:'10px 18px',minHeight:44,marginTop:0},onClick:p.onEndings},'엔딩')));
}
function Tutorial(p){
...
function RewardScreen(p){
  ...
  var av=pickN(REWARDS,count);
  return h('div',{className:'screen'},h(Stats,{stats:p.stats}),h('div',{style:{flex:1,width:'100%',maxWidth:420,padding:'20px 0',overflowY:'auto'}},h('div',{style:{fontFamily:"'Share Tech Mono',monospace",fontSize:12,color:'#f0a030',letterSpacing:2,textAlign:'center',marginBottom:6}},'ORACLE 자원 배분'),h('div',{style:{fontSize:13,color:'#ccddcc',textAlign:'center',marginBottom:20}},count===4?'4개 중 1개를 선택하십시오':count===3?'자원 부족 — 3개 중 선택':count===2?'경고 — 2개 중 선택':count===1?'긴급 상황 — 1건':'선택'),av.map(function(r){return h('div',{key:r.id,onClick:function(){p.onPick(r)},style:{background:'#0d1a0d',border:'1px solid #1a3a1a',borderRadius:4,padding:'14px 16px',marginBottom:10,cursor:'pointer'}},h('div',{style:{fontSize:14,color:'#33ff33',fontWeight:'bold',marginBottom:6}},r.title),h('div',{style:{fontSize:12,color:'#88aa88',marginBottom:8,lineHeight:1.6}},r.desc),h('div',{style:{display:'flex',justifyContent:'space-between',fontSize:12}},h('span',{style:{color:'#33ff33'}},'▲ '+r.benefit),h('span',{style:{color:'#ff6644'}},'▼ '+r.cost)))})));
}
function Dialogue(p){
  ...
  var fxTags=function(fx){if(!fx)return null;var tags=[];['c','r','t','o'].forEach(function(k){if(fx[k]&&fx[k]!==0){tags.push({key:k,val:fx[k],name:SN[k].l,ic:SN[k].ic})}});if(!tags.length)return null;return h('div',{style:{display:'flex',gap:10,marginTop:10,paddingLeft:16,flexWrap:'wrap',animation:'fadeIn 0.5s ease'}},tags.map(function(t){var pos=t.val>0;return h('span',{key:t.key,style:{fontFamily:"'Share Tech Mono',monospace",fontSize:11,color:pos?'#50ff50':'#ff4444',textShadow:'0 0 6px '+(pos?'rgba(80,255,80,0.4)':'rgba(255,68,68,0.4)'),letterSpacing:1}},t.ic+t.name+(pos?'+':'')+t.val)}))};
  return h('div',{className:'screen'},h('div',{style:{flex:1,width:'100%',maxWidth:420,padding:'20px 0',display:'flex',flexDirection:'column'}},
    h('div',{style:{textAlign:'center',marginBottom:16,flexShrink:0}},portrait&&h('img',{src:portrait,className:'portrait',alt:d.char,style:{width:110,height:110}}),h('div',{style:{fontSize:16,color:'#f0a030',fontWeight:'bold'}},d.char),h('div',{style:{fontFamily:"'Share Tech Mono',monospace",fontSize:11,color:'#1a6a1a',marginTop:4}},d.role)),
    h('div',{style:{flex:1,display:'flex',flexDirection:'column',justifyContent:'center',padding:'0 8px',overflowY:'auto',minHeight:0}},
      d.lines.slice(0,li).map(function(l,i){return h('div',{key:i,style:{fontSize:14,lineHeight:1.9,color:'#ccddcc',marginBottom:8,paddingLeft:16,borderLeft:'2px solid #1a5a1a'}},String(l))}),
      chosen&&chosen.reply&&h('div',{style:{fontSize:14,lineHeight:1.9,color:'#f0a030',marginTop:12,paddingLeft:16,borderLeft:'2px solid #8a6020',transition:'opacity 0.3s'}},rTxt,!rDone&&h('span',{style:{animation:'blink 1s infinite',marginLeft:2}},'▌')),
      rDone&&chosen&&fxTags(chosen.fx)),
    sc&&!chosen&&h('div',{style:{display:'flex',flexDirection:'column',gap:10,alignItems:'center',paddingBottom:20,flexShrink:0}},d.choices.map(function(c,i){var isMe=picked===i;var isOther=picked>=0&&picked!==i;return h('button',{key:i,className:'btn'+(i===0?' btn-amber':''),style:{fontSize:13,padding:'10px 20px',width:'80%',maxWidth:300,marginTop:0,opacity:isOther?0.2:1,transform:isMe?'scale(1.03)':'scale(1)',background:isMe?(i===0?'rgba(240,160,48,0.15)':'rgba(51,255,51,0.1)'):'none',boxShadow:isMe?'0 0 20px '+(i===0?'rgba(240,160,48,0.3)':'rgba(51,255,51,0.2)'):'none',transition:'all 0.3s ease',pointerEvents:picked>=0?'none':'auto'},onClick:function(){handlePick(c,i)}},c.label)}))
  ));
}
