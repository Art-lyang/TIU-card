...
function App(){
  ...
  var modTrust=function(char,delta){setTrust(function(prev){var key={"\uc11c\ud558\uc740":"haeun","\uac15\ub3c4\uc724":"doyun","\uc724\uc138\uc9c4":"sejin","\uc784\uc7ac\ud601":"jaehyuk","\ub9c8\ub974\ucfe0\uc2a4 \ubca0\ubc84":"weber","\ub2c9 \ud3ec\uc2a4\ud130":"foster","\ubc15\uc18c\uc601":"soyoung",haeun:"haeun",doyun:"doyun",sejin:"sejin",jaehyuk:"jaehyuk",weber:"weber",foster:"foster",soyoung:"soyoung"}[char];if(!key)return prev;var next={};for(var k in prev)next[k]=prev[k];next[key]=Math.max(0,Math.min(100,(prev[key]||0)+delta));Save.set('ts_trust',next);
    return next})};
  var journalBond=function(ch,tone){try{if(typeof Journal==='undefined'||!ch)return;var K={"서하은":"haeun","강도윤":"doyun","윤세진":"sejin","임재혁":"jaehyuk","박소영":"soyoung",haeun:"haeun",doyun:"doyun",sejin:"sejin",jaehyuk:"jaehyuk",soyoung:"soyoung"}[ch];if(!K)return;Journal.push({t:'bond',id:K,d:tone,day:(stats&&stats.day)||0,act:act})}catch(_e){}};
  ...
  if(phase==='evening'){return withOracleLink(h(React.Fragment,null,h(EveningChat2,{day:stats.day,act:act,logs:logs,gi:gi,trust:trust,facility:facility,sessions:sessions,usedEvening:usedEvening,onMarkEvening:function(key){setUsedEvening(function(p){if(p.indexOf(key)>=0)return p;var n=p.concat([key]);Save.saveUsedEvening(n);return n})},onChat:function(cn){modTrust(cn,1);journalBond(cn,'warm')},onResponse:function(cn,delta){modTrust(cn,delta);journalBond(cn,delta<0?'cold':'warm')},onDone:function(){hEvening()},onTrustMod:function(ck,v){modTrust(ck,v);journalBond(ck,v<0?'cold':'warm')},onGiMod:function(v){setGi(function(g){var ng=g+v;persistGame(stats,ng,act,actFlags,transRoute,cooldowns,recentCards,ct,chainQueue);return ng})},onLog:function(id){tryUnlock(id)},onOpenEvidence:function(){setShowEvidence(true)}}),
