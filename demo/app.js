...
function App(){
  ...
  var modTrust=function(char,delta){setTrust(function(prev){var key={"\uc11c\ud558\uc740":"haeun","\uac15\ub3c4\uc724":"doyun","\uc724\uc138\uc9c4":"sejin","\uc784\uc7ac\ud601":"jaehyuk","\ub9c8\ub974\ucfe0\uc2a4 \ubca0\ubc84":"weber","\ub2c9 \ud3ec\uc2a4\ud130":"foster","\ubc15\uc18c\uc601":"soyoung",haeun:"haeun",doyun:"doyun",sejin:"sejin",jaehyuk:"jaehyuk",weber:"weber",foster:"foster",soyoung:"soyoung"}[char];if(!key)return prev;var next={};for(var k in prev)next[k]=prev[k];next[key]=Math.max(0,Math.min(100,(prev[key]||0)+delta));Save.set('ts_trust',next);
    return next})};
  var journalBond=function(ch,tone,dayArg){try{if(typeof Journal==='undefined'||!ch)return;var K={"서하은":"haeun","강도윤":"doyun","윤세진":"sejin","임재혁":"jaehyuk","박소영":"soyoung",haeun:"haeun",doyun:"doyun",sejin:"sejin",jaehyuk:"jaehyuk",soyoung:"soyoung"}[ch];if(!K)return;Journal.push({t:'bond',id:K,d:tone,day:(typeof dayArg==='number'?dayArg:((stats&&stats.day)||0)),act:act})}catch(_e){}};
  ...
  var nextCard=function(s,g,lg,cq,curAct,cdOverride,rcOverride,trOverride,facOverride){var a=curAct||act;var useCd=cdOverride||cooldowns;var useRecent=rcOverride||recentCards;var useRoute=typeof trOverride==='string'?trOverride:transRoute;var useFacility=facOverride||facility;var liveLg=getLiveLogs(lg);if(window.TIU_DEMO&&TIU_DEMO.enabled&&(!cq||!cq.length)&&s&&s.day>=(TIU_DEMO.forceMissionDay||7)&&liveLg.indexOf('ONCE-C-096')<0&&typeof CARD_BY_ID!=='undefined'&&CARD_BY_ID['C-096']){var _dfm=CARD_BY_ID['C-096'];setCurCard(_dfm);setRecentCards(function(p){var _n=(rcOverride||p).concat([_dfm.id]);return _n.length>60?_n.slice(_n.length-60):_n});unlockCardInput();freshCardRef.current=true;return}if(cq&&cq.length>0){setCurCard(cq[0]);setChainQueue(cq.slice(1))}else{var c=drawCard(s,g,liveLg,useCd,useRecent,a,useRoute,useFacility);if(!c){c={id:'SYS-FALLBACK',msg:tt('app.fallbackCardMsg',null,'[ORACLE: 데이터 스트림 일시 중단]\n\n통신 복구 대기 중...'),left:{label:tt('app.fallbackCardLeft',null,'대기'),fx:{},g:0},right:{label:tt('app.fallbackCardRight',null,'재접속 시도'),fx:{},g:0}}}setCurCard(c);setRecentCards(function(p){var base=rcOverride||p;var n=base.concat([c.id]);return n.length>60?n.slice(n.length-60):n})}unlockCardInput();freshCardRef.current=true};
  ...
  var doBriefing=function(newAct,s,route,flagsOverride){
    if(typeof window!=='undefined'&&window.TS_DEMO&&newAct>=3){setPhase('demoend');return} // 데모 게이트: Act3 진입 관문에서 컷 (demo-flag.js)
  ...
  var swipe=function(dir){
    ...
    if(ch.automation==='on')tryUnlock('LOG-AUTO-ON');
    else if(ch.automation==='off')tryUnlock('LOG-AUTO-OFF');
  ...
  var startNewCampaign=function(showTutorial){goingRef.current=false;
    ...
    Save.del('ts_activeMission');Save.del('ts_recentRewards');Save.del('ts_recentNews');Save.del('ts_resourceReserveUsed');Save.del('ts_lastLog');Save.del('ts_journal');Save.del('ts_onboardBriefDone');
  ...
  if(phase==='demoend'){var _den=(typeof getLocale==='function'&&getLocale()==='en');return h('div',{className:'screen',style:{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center',minHeight:'100vh',padding:'34px 22px',gap:15}},h('div',{style:{font:'12px monospace',letterSpacing:'2px',color:'#39d98a',opacity:.85}},_den?'DEMO COMPLETE':'데모 체험 완료'),h('div',{style:{font:'700 22px/1.45 sans-serif',color:'#eaf4ee',maxWidth:360}},_den?'You’ve reached the end of Act 2.':'Act 2까지 체험하셨습니다.'),h('div',{style:{font:'14px/1.7 sans-serif',color:'#9fb3aa',maxWidth:346}},_den?'The full version continues through Acts 3–4 — the crisis, the escape, and every ending path.':'풀버전에서는 Act 3–4의 위기와 탈출, 모든 엔딩 분기가 이어집니다.'),h('button',{onClick:function(){var _href=(window.TIU_DEMO&&TIU_DEMO.fullVersionHref)||'../index.html';try{window.location.href=_href}catch(e){}},style:{marginTop:10,padding:'13px 26px',font:'700 15px sans-serif',color:'#04140d',background:'#39d98a',border:'none',borderRadius:8,cursor:'pointer'}},_den?'Get the Full Version':'풀버전으로 계속하기'),h('button',{onClick:function(){try{setPhase('menu')}catch(e){}},style:{padding:'9px 18px',font:'13px sans-serif',color:'#7f938a',background:'transparent',border:'1px solid rgba(127,147,138,.4)',borderRadius:6,cursor:'pointer'}},_den?'Back to menu':'메인 메뉴로'));}  if(phase==='boot')return h(Boot,{sessions:sessions,onBoot:function(){BGM.startBootLoop()},onDone:function(){BGM.stopBootLoop();BGM.start();setPhase('menu')}});
  ...
  if(phase==='onboard'&&typeof CommandBrief!=='undefined')return h(CommandBrief,{onDone:function(){try{Save.set('ts_onboardBriefDone',true)}catch(e){}setPhase('game')}});
  if(phase==='demoend')return h(React.Fragment,null,typeof DemoEndScreen!=='undefined'?h(DemoEndScreen,{stats:stats,gi:gi,onMenu:returnToMainMenu}):null,renderToastBar());
  if(phase==='briefing'){
  ...
  if(phase==='evening'){return withOracleLink(h(React.Fragment,null,h(EveningChat2,{day:stats.day,act:act,logs:logs,gi:gi,trust:trust,facility:facility,sessions:sessions,usedEvening:usedEvening,onMarkEvening:function(key){setUsedEvening(function(p){if(p.indexOf(key)>=0)return p;var n=p.concat([key]);Save.saveUsedEvening(n);return n})},onChat:function(cn){modTrust(cn,1);journalBond(cn,'warm',Math.max(1,(stats.day||1)-1))},onResponse:function(cn,delta){modTrust(cn,delta);journalBond(cn,delta<0?'cold':'warm',Math.max(1,(stats.day||1)-1))},onDone:function(){hEvening()},onTrustMod:function(ck,v){modTrust(ck,v);journalBond(ck,v<0?'cold':'warm',Math.max(1,(stats.day||1)-1))},onGiMod:function(v){setGi(function(g){var ng=g+v;persistGame(stats,ng,act,actFlags,transRoute,cooldowns,recentCards,ct,chainQueue);return ng})},onLog:function(id){tryUnlock(id)},onOpenEvidence:function(){setShowEvidence(true)}}),
  ...
  if(phase==='dialogue'&&curDlg)return withOracleLink(h(ConnectingGate,{key:curDlg.id||curDlg.char,label:getLocale()==='en'?'[ RECEIVING SIGNAL ... ]':'[ 신호 수신 중 ... ]',preload:[typeof CHAR_IMG!=='undefined'?CHAR_IMG[curDlg.char]:null]},h(Dialogue,{dialogue:curDlg,onChoice:hDlg})));
  if(phase==='mission'&&curMission)return withOracleLink(h(ConnectingGate,{key:curMission,ms:950,label:getLocale()==='en'?'[ CONNECTING TO FIELD ... ]':'[ 현장과 연결 중 ... ]',preload:[(typeof MISSIONS!=='undefined'&&MISSIONS[curMission]&&MISSIONS[curMission].hero)||null,typeof getMissionImage==='function'?getMissionImage(curMission):null]},h(FieldMission,{missionId:curMission,trust:trust,logs:logs,act:act,onComplete:(debugMissionRef.current?hMissionDebug:hMission)})));
