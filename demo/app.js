...
function DevMissionLauncher(p){
  ...
  var panel=h('div',{style:{position:'fixed',left:10,bottom:46,zIndex:99999,width:330,maxHeight:'72vh',overflowY:'auto',padding:'10px 12px',background:'rgba(6,14,13,.97)',border:'1px solid rgba(57,217,138,.5)',borderRadius:5,boxShadow:'0 6px 24px rgba(0,0,0,.6)'}},
    ...
    p.onLaunchSting?h('button',{key:'sting-bs',onClick:function(){p.onLaunchSting('brainseeker')},style:{display:'block',width:'100%',textAlign:'left',font:'11px monospace',padding:'7px 9px',margin:'3px 0',cursor:'pointer',background:'rgba(22,14,6,.9)',color:'#ff8c2b',border:'1px solid rgba(255,140,43,.45)',borderRadius:3}},h('span',{style:{opacity:.7,marginRight:8}},'[CCTV]'),'브레인시커 → M-E01'):null,
...
function App(){
  ...
  var launchDebugSting=function(key){var mid='M-E01';if(typeof MISSIONS==='undefined'||!MISSIONS[mid])return;debugMissionRef.current=true;setShowDevPanel(false);setCurMission(mid);setCctvSting(key||'brainseeker')};
