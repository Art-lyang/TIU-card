...
// i18n ?ы띁: tt(path, params, fallback) ??t() ?놁쑝硫??대갚 諛섑솚
...
function ScenarioHub(p){
  ...
  var scenarios=[
    {id:'main',title:'TERMINAL SESSION',sub:'MISSION: KOREAN BRANCH STABILIZATION',desc:'?쒓뎅 吏遺 遊됱뇙 援ъ뿭 愿由??쒕굹由ъ삤',active:true,img:IMG.hub_main},
    {id:'dlc_green',title:'GREEN THRESHOLD',sub:'MISSION: SOVARI BLIND ZONE',desc:'?꾪봽由ъ뭅 ?뚮컮由??먰뿀 ??EV-誇 ?붾쪟 援ъ뿭 ?먯궗',active:false,img:IMG.hub_dlc_green},
    {id:'dlc_north',title:'NORTHERN FRONT',sub:'MISSION: SITE-7/13',desc:'?ъ떆??遺곴레沅????좏샇 李⑤떒 援ъ뿭 移⑦닾',active:false,img:IMG.hub_dlc_north}
  ];
  ...
  if(mode==='main') return h('div',{className:'boot',style:{justifyContent:'flex-start',padding:'8px 0',gap:0,overflowY:'auto'}},
    entryImg&&h('div',{style:{width:'100%',maxWidth:440,flexShrink:0,position:'relative',borderRadius:4,overflow:'hidden'}},
      ...
      h('div',{style:{position:'absolute',inset:0,display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',padding:'20px 24px',gap:6}},
        ...
        h('div',{style:Object.assign({},mono,{fontSize:15,color:'#f0a030',letterSpacing:2,marginTop:4,textAlign:'center'})},isKo?'?ㅻ씪???쒓뎅吏遺 ?덉젙??:'MISSION: KOREAN BRANCH'),
        ...
        h('div',{style:{marginTop:6,textAlign:'center',lineHeight:1.8}},
          h('div',{style:Object.assign({},mono,{fontSize:9,color:'rgba(var(--ui-rgb),.35)'})},isKo?'媛뺤썝???ㅻ씪???쒓뎅吏遺':'ORACLE KOREA BRANCH / GANGWON SECTOR'),
    ...
    h('div',{style:{display:'flex',flexDirection:'column',gap:10,alignItems:'center',marginTop:4,flexShrink:0,paddingBottom:20}},
      p.hasSave&&h('button',{className:'btn btn-amber',style:{minWidth:220},onClick:p.onContinue},tt('hub.continue',null,isKo?'[ ?댁뼱?섍린 ]':'[ Continue ]')),
      h('button',{className:'btn',style:{minWidth:220,borderColor:p.hasSave?'rgba(var(--ui-rgb),.4)':'#f0a030',color:p.hasSave?'var(--ui)':'#f0a030'},onClick:p.hasSave?p.onNew:p.onTutorial},p.hasSave?tt('hub.newGame',null,isKo?'[ ?덇쾶??]':'[ New Game ]'):tt('hub.start',null,isKo?'[ 寃뚯엫?쒖옉 ]':'[ Start ]')),
      p.hasSave&&h('button',{className:'btn',style:{minWidth:220,fontSize:11,opacity:0.5},onClick:p.onTutorial},tt('hub.replayTutorial',null,isKo?'[ ?쒗넗由ъ뼹 ?ㅼ떆?섍린 ]':'[ Replay Tutorial ]')),
      h('div',{onClick:function(){setMode('select')},style:Object.assign({},mono,{fontSize:10,color:'rgba(var(--ui-rgb),.3)',cursor:'pointer',marginTop:6})},tt('hub.backToSelect',null,isKo?'???ㅻ줈媛湲?:'??Back to Scenario Select'))));
  ...
  return h('div',{className:'boot',style:{justifyContent:'flex-start',padding:'8px 0',gap:0,overflowY:'auto'}},
    ...
    sc.active&&h('button',{className:'btn btn-amber',style:{flexShrink:0},onClick:function(){setMode('main')}},tt('hub.enter',null,'[ 吏꾩엯 ]')),
    !sc.active&&h('div',{style:Object.assign({},mono,{fontSize:10,color:'rgba(var(--ui-rgb),.2)',textAlign:'center',flexShrink:0})},tt('hub.exploreSwipe',null,'???ㅼ??댄봽?섏뿬 ?쒕굹由ъ삤 ?먯깋 ??)),
...
function MainMenu(p){
  ...
  return h('div',{className:'boot',style:{justifyContent:'center',gap:0}},
    ...
    h('div',{style:{display:'flex',flexDirection:'column',gap:10,alignItems:'center'}},
      h('button',{className:'btn btn-amber',style:{minWidth:240,fontSize:13},onClick:p.onPlay},tt('menu.startGame',null,'[ 寃뚯엫 ?쒖옉 ]')),
      p.hasSave&&h('button',{className:'btn',style:{minWidth:240},onClick:p.onContinue},tt('menu.continue',null,'[ ?댁뼱?섍린 ]')),
      h('button',{className:'btn',style:{minWidth:240},onClick:p.onArchive},tt('settings.archive',null,'?꾩뭅?대툕')),
      h('button',{className:'btn',style:{minWidth:240},onClick:p.onLogs},tt('gameOver.logs',null,'濡쒓렇')),
...
function Stats(p){
  var sm=[{k:'c',l:tt('stats.c',null,'??')},{k:'r',l:tt('stats.r',null,'??')},{k:'t',l:tt('stats.t',null,'??')},{k:'o',l:tt('stats.o',null,'??')}];
  var pv=p.preview||{};
  var locale=(window.TS_I18N&&window.TS_I18N.getLocale&&window.TS_I18N.getLocale())||'ko';
  var isKo=locale==='ko';
...
function CardC(p){
  ...
  // ?숈쟻 ?띿꽦 吏????timer/label???⑥닔硫??몄텧?댁꽌 ?몄뀡 ?잛닔 ??諛섏쁺
  ...
  // ?먥븧??移대뱶 ??대㉧ (card.timer 珥??⑥쐞) ??留뚮즺 ???ㅻⅨ履??먮룞 ?좏깮 ?먥븧??
  ...
  // ?좏깮吏 ?뺤젙 ??留ㅻ돱???ㅻ씪?댁감???꾨떂) + replyMsg ?덉쑝硫??좎뒪???몄텧 ??onSwipe
  ...
  useEffect(function(){
    var onKey=function(e){
      ...
      if(shouldBlock){
        var bmsgs=card.oracleBlockMsgs||['[ORACLE: 紐낅졊 嫄곕? 媛먯? ???ы솗???붿껌]','[ORACLE: ?쒖쓳 ?꾨줈?좎퐳 ?쒖꽦??以?','[ORACLE: 寃쎄퀬 ??遺덉씠??湲곕줉 以?'];
  ...
  var hE=function(){setDragging(false);if(p.onPreview)p.onPreview(null);if(dir){var shouldBlock=card.oracleBlock&&blockCount<card.oracleBlock&&dir===(card.oracleBlockDir||'left');if(shouldBlock){setDx(0);var bmsgs=card.oracleBlockMsgs||['[ORACLE: 紐낅졊 嫄곕? 媛먯? ???ы솗???붿껌]','[ORACLE: ?쒖쓳 ?꾨줈?좎퐳 ?쒖꽦??以?','[ORACLE: 寃쎄퀬 ??遺덉씠??湲곕줉 以?'];var bmsg=bmsgs[Math.min(blockCount,bmsgs.length-1)];setBlockCount(blockCount+1);if(p.onOracleBlock)p.onOracleBlock(bmsg);setTimeout(function(){setShaking(true);setTimeout(function(){setShaking(false)},600)},60);}else{setDx(0);performSwipe(dir)}}else setDx(0)};
  var pcClass=card.priority==='???' card-p-high':card.priority==='以??' card-p-mid':' card-p-low';
  if(card.glitch)pcClass+=' card-glitch';
  var plbl=card.priority==='???tt('card.priorityShort.high',null,'????):card.priority==='以??tt('card.priorityShort.mid',null,'以???):tt('card.priorityShort.low',null,'??);
  ...
  var SN={c:tt('stats.c',null,'遊됱뇙'),r:tt('stats.r',null,'?먯썝'),t:tt('stats.t',null,'?좊ː'),o:tt('stats.o',null,'?됯?')};
  var fxHint=function(fx){if(!fx)return null;var tags=[];['c','r','t','o'].forEach(function(k){var v=(fx[k]||0);var abs=Math.abs(v);if(v>0)tags.push(h('span',{key:k,style:{color:'var(--ui)'}},SN[k]+(abs>=2?'?묅넁':'??)));if(v<0)tags.push(h('span',{key:k,style:{color:'rgba(255,141,97,.9)'}},SN[k]+(abs>=2?'?볛넃':'??)))});return tags.length?tags:null};
  ...
  return h('div',{style:{flex:1,width:'100%',maxWidth:440,position:'relative',display:'flex',flexDirection:'column',minHeight:0}},
    ...
    h('div',{className:'card-panel'+pcClass,style:{transform:shaking?'none':'translateX('+tx+'px) rotate('+(tx*0.04)+'deg)',animation:shaking?'oracleShake 0.6s ease':'none',transition:dragging||shaking?'none':'transform 0.3s ease',opacity:chosen?0:1,touchAction:'none',WebkitUserSelect:'none',userSelect:'none'},
      ...
      h('div',{className:'card-hdr'},h('span',{className:'card-hdr-l'},card.glitch?'ERR:0x8F2A':card.isFacilityProposal?tt('card.facilityExpansion',null,'?쒖꽕 ?뺤옣'):tt('card.oracleComm',null,'ORACLE ?듭떊')),h('span',{className:'card-hdr-r'},card.glitch?'?댿뻽?댿뻽?댿뻽':tt('card.priority',{priority:plbl},'?곗꽑?쒖쐞: '+plbl))),
        ...
        return paras.map(function(para,pi){
          ...
          return h('div',{key:pi,style:{marginBottom:pi<paras.length-1?10:0}},
            lines.map(function(line,li){
              ...
              // [ORACLE ...] ?愿꾪샇 ?ㅽ???
              if(s.match(/^\[ORACLE[\s:竊?/))return h('div',{key:li,style:{color:'#f0a030',fontFamily:"'Share Tech Mono',monospace",fontSize:12,padding:'6px 10px',margin:'4px 0',background:'rgba(240,160,48,.06)',borderLeft:'2px solid rgba(240,160,48,.3)',borderRadius:2}},s);
              // ORACLE 遺꾩꽍/沅뚭퀬/?듭떊 ??(?愿꾪샇 ?녿뒗 ORACLE ?쇱씤)
              ...
              // 罹먮┃????????대쫫(+吏곸콉/?섏떇?? + 肄쒕줎
              if(s.match(/^(?쒗븯?|媛뺣룄???ㅼ꽭吏??꾩옱??諛뺤냼??留덈Ⅴ荑좎뒪 踰좊쾭|???ъ뒪???ъ뒪??([\s\u00A0][\uAC00-\uD7A3A-Za-z\s]*)?[竊?]/))return h('div',{key:li,style:{color:'#7ec8e3',padding:'3px 0',margin:'2px 0'}},s);
              // "?몄슜臾? ???띾뵲?댄몴濡??쒖옉?섎뒗 ?낅┰ ???
      ...
      (leftFx||rightFx)&&h('div',{style:{marginTop:'auto',padding:'10px 0 6px',borderTop:'1px solid rgba(var(--ui-rgb),.08)'}},
        h('div',{style:{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)',fontFamily:"'Share Tech Mono',monospace",fontSize:9,columnGap:10,rowGap:3,alignItems:'center'}},
          h('div',{style:{display:'flex',gap:6,alignItems:'center',justifyContent:'flex-start',textAlign:'left',opacity:0.7}},h('span',{style:{color:'rgba(var(--ui-rgb),.5)',fontSize:9}},'←'),leftFx||h('span',{style:{color:'rgba(var(--ui-rgb),.3)'}},'··')),
          h('div',{style:{display:'flex',gap:6,alignItems:'center',justifyContent:'flex-end',textAlign:'right',opacity:0.7}},rightFx||h('span',{style:{color:'rgba(var(--ui-rgb),.3)'}},'··'),h('span',{style:{color:'rgba(var(--ui-rgb),.5)',fontSize:9}},'→')))),
      h('div',{style:{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)',paddingTop:8,borderTop:'1px solid rgba(var(--ui-rgb),.1)',fontFamily:"'Share Tech Mono',monospace",fontSize:9,pointerEvents:'none',columnGap:10,alignItems:'center'}},
        ...
        h('span',{style:{color:'rgba(var(--ui-rgb),.45)',display:'block',textAlign:'right'}},rightLabel+' →'))
    ));
}
function News(p){
  ...
  var parseHL=function(raw){var s=String(raw||'');var isGl=s.indexOf('遺꾨쪟 ?ㅻ쪟')>=0;var isDel=s.indexOf('??젣??)>=0;if(isGl||isDel)return{tag:'REDACTED',text:s,gl:true};if(s.indexOf('[?댁쇅]')>=0){fIdx++;return{tag:'FOREIGN-0'+fIdx,text:s.replace('[?댁쇅] ',''),gl:false}}if(s.indexOf('[援?궡]')>=0){dIdx++;return{tag:'DOMESTIC-0'+dIdx,text:s.replace('[援?궡] ',''),gl:false}}return{tag:'INTEL-01',text:s,gl:false}};
  var st=p.stats||{};var gi=p.gi||0;var act=p.act||1;
  var AP={h:["?댁쁺 ?⑥쑉 ?묓샇. ?꾪뻾 ?좎? 沅뚭퀬.","ORACLE 沅뚭퀬 ?댄뻾瑜??곗닔. ?쒓뎅 吏遺 ?깃낵 ?곸쐞沅?","吏?섍? ?먮떒 ?좊ː???믪쓬. ???댁쁺 諛⑹묠 ?좎?.","湲곗? ?덉젙???뺤씤. 異붽? 沅뚰븳 遺??寃??以?"],m:["?댁쁺 ?덉젙. ?쇰? 鍮꾪몴以 ?⑦꽩 媛먯?.","?꾨컲???덉젙. ?낅┰???먮떒 鍮덈룄 ?뚰룺 利앷?.","湲곗? ?댁쁺 ?뺤긽 踰붿쐞. ?쇰? 吏??蹂??二쇱떆 以?","ORACLE 沅뚭퀬 ?댄뻾瑜?蹂댄넻. 愿李?吏??"],l:["鍮꾪몴以 ?먮떒 鍮덈룄 利앷?. 紐⑤땲?곕쭅 媛뺥솕.","?낆옄???섏궗寃곗젙 ?⑦꽩 媛먯?. 遺꾩꽍 以?","ORACLE 沅뚭퀬 ?댄깉 鍮덈룄 ?곸듅. 湲곕줉 以?","?댁쁺 ?곗씠??遺꾩꽍 ??鍮꾪몴以 ??ぉ ?ㅼ닔 ?뺤씤."],v:["鍮꾪몴以 ?댁쁺 ?⑦꽩 ?ㅼ닔 媛먯?. 二쇱쓽 ?붾쭩.","吏?섍? ?좊ː 吏???섎씫 以? ?ы룊媛 ?덉젙.","ORACLE 沅뚭퀬 臾댁떆 鍮덈룄 ?꾪뿕 ?섏? ?묎렐.","?댁쁺 ?댁긽 媛먯?. 蹂몃? 蹂닿퀬 寃??以?"]};
  ...
  return h('div',{className:'oracle-card',style:{width:'100%',maxWidth:440,padding:'20px 22px 16px',cursor:'default',marginTop:'auto',marginBottom:'auto',display:'flex',flexDirection:'column',maxHeight:'calc(100vh - 60px)',overflow:'hidden'}},
    ...
    h('div',{style:{flex:1,overflowY:'auto',minHeight:0}},
      h('div',{style:{marginBottom:12,padding:'8px 0',borderBottom:'1px solid rgba(var(--ui-rgb),.08)'}},
        ...
        h('div',{style:{display:'flex',flexDirection:'column',gap:4}},
          statBar('c',(st.c==null?50:st.c),tt('stats.c',null,'遊됱뇙')),statBar('r',(st.r==null?60:st.r),tt('stats.r',null,'?먯썝')),statBar('t',(st.t==null?50:st.t),tt('stats.t',null,'?좊ː')),statBar('o',(st.o==null?40:st.o),tt('stats.o',null,'?됯?'))),
    ...
    shown>=p.headlines.length&&h('div',{style:{textAlign:'center',marginTop:14,paddingTop:10,borderTop:'1px solid rgba(var(--ui-rgb),.12)',flexShrink:0}},
      h('button',{className:'oracle-card__execute',style:{minWidth:200},onClick:p.onContinue},tt('news.nextCycle',null,'[ ?ㅼ쓬 ?ъ씠??吏꾪뻾 ]'))));
}
function NewsReport(p){
  ...
  var parseHL=function(raw){
    ...
    var isGl=s.indexOf('遺꾨쪟 ?ㅻ쪟')>=0;
    var isDel=s.indexOf('??젣??)>=0;
    if(isGl||isDel||type==='redacted')return{tag:'REDACTED',text:body,gl:true};
    if(type==='foreign'||s.indexOf('[?댁쇅]')>=0){fIdx++;return{tag:'FOREIGN-0'+fIdx,text:body.replace('[?댁쇅] ','').replace('[OVERSEAS] ',''),gl:false}}
    if(type==='domestic'||s.indexOf('[援?궡]')>=0){dIdx++;return{tag:'DOMESTIC-0'+dIdx,text:body.replace('[援?궡] ','').replace('[DOMESTIC] ',''),gl:false}}
...
function NewsReport3(p){
  ...
  var parseHL=function(raw){
    ...
    var body=view&&view.text?view.text:s;
    var type=view&&view.type?view.type:null;
    var isGl=type==='redacted'||s.indexOf('遺꾨쪟 ?ㅻ쪟')>=0||s.indexOf('[??젣??')>=0;
    if(isGl)return{tag:'REDACTED',text:body,gl:true};
    if(type==='foreign'||s.indexOf('[?댁쇅]')===0||s.indexOf('[OVERSEAS]')===0){fIdx++;return{tag:'FOREIGN-0'+fIdx,text:body.replace('[?댁쇅] ','').replace('[OVERSEAS] ',''),gl:false}};
    if(type==='domestic'||s.indexOf('[援?궡]')===0||s.indexOf('[DOMESTIC]')===0){dIdx++;return{tag:'DOMESTIC-0'+dIdx,text:body.replace('[援?궡] ','').replace('[DOMESTIC] ',''),gl:false}};
    return{tag:'INTEL-01',text:body,gl:false};
  };
  var st=p.stats||{};var gi=p.gi||0;var act=p.act||1;
  var AP={
    ...
    h('div',{style:{flex:1,overflowY:'auto',minHeight:0}},
      ...
      (function(){
        ...
        return h('div',{style:{marginBottom:12,padding:'8px 0',borderBottom:'1px solid rgba(var(--ui-rgb),.08)'}},
          h('div',{style:{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:'rgba(var(--ui-rgb),.55)',letterSpacing:1,marginBottom:6}},tt('news.sectionSituation',null,'[SITUATION REPORT]')),
          sitLines.map(function(line,i){
            var isWarn=line.indexOf('寃쎄퀬')>=0||line.indexOf('warning')>=0||line.indexOf('CRITICAL')>=0;
            return h('div',{key:'sit-'+i,style:{fontSize:11,lineHeight:1.6,color:isWarn?'#ff8844':'var(--ui-text)',fontFamily:"'Share Tech Mono',monospace",padding:'2px 0',animation:'fadeIn 0.4s ease'}},line)
          }))
      })(),
      h('div',{style:{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:'rgba(var(--ui-rgb),.55)',letterSpacing:1,marginBottom:6}},tt('news.sectionIntel',null,'[INTEL BRIEFING]')),
...
function GameOver(p){
  ...
  var msg=p.gi>50?tt('gameOver.msgHigh',null,"?붿썝???뚯떊??蹂듬Т??媛먯궗?쒕┰?덈떎."):p.gi>25?tt('gameOver.msgMid',null,"?몄뀡??醫낅즺?⑸땲?? 寃곌낵媛 湲곕줉?섏뿀?듬땲??"):tt('gameOver.msgLow',null,"鍮꾪몴以 ?댁쁺 ?⑦꽩 媛먯?. ?몄뀡 ?곗씠??遺꾩꽍 以?..");
  var narr=p.endNarr;
  ...
  var btns=h('div',{style:{flexShrink:0,display:'flex',flexDirection:'column',alignItems:'center',gap:10,paddingBottom:20}},
    h('button',{className:'btn btn-amber',onClick:p.onRestart},tt('gameOver.restart',null,'[ ?몄뀡 ?ш컻????ACT 1 ]')),
    p.canNgPlus&&h('button',{className:'btn',style:{fontSize:12,padding:'10px 18px',minHeight:44,marginTop:0,borderColor:'#c080ff',color:'#c080ff'},onClick:p.onNewGamePlus},tt('gameOver.ngPlus',null,'[ NEW GAME+ ??媛뺥솕 ?쒖옉 ]')),
    h('div',{style:{display:'flex',gap:10}},h('button',{className:'btn',style:{fontSize:12,padding:'10px 18px',minHeight:44,marginTop:0},onClick:p.onLogs},tt('gameOver.logs',null,'湲곕줉')),h('button',{className:'btn',style:{fontSize:12,padding:'10px 18px',minHeight:44,marginTop:0},onClick:p.onArchive},tt('gameOver.archive',null,'?꾩뭅?대툕')),h('button',{className:'btn',style:{fontSize:12,padding:'10px 18px',minHeight:44,marginTop:0},onClick:p.onEndings},tt('gameOver.endings',null,'?붾뵫'))));
  ...
  var goImg=null;if(p.reason&&p.reason.indexOf('遊됱뇙')>=0)goImg=IMG.ending_C_c;else if(p.reason&&p.reason.indexOf('?먯썝')>=0)goImg=IMG.ending_C_r;else if(p.reason&&p.reason.indexOf('?좊ː')>=0)goImg=IMG.ending_C_t;else if(p.reason&&(p.reason.indexOf('?됯?')>=0||p.reason.indexOf('?묒냽')>=0))goImg=IMG.ending_C_o;
  return h('div',{className:'boot',style:{overflowY:'auto'}},goImg&&h('img',{src:goImg,alt:'Game Over',style:imgStyle}),h('div',{style:{fontSize:13,lineHeight:1.9,maxWidth:420,width:'100%',textAlign:'center'}},h('div',{className:'go-title'},'??? SESSION #'+(p.sessions+1)+' TERMINATED ???'),h('div',{className:'go-reason'},p.reason),h('div',{className:'go-section'},tt('gameOver.reportSection',null,'?? ORACLE 理쒖쥌 蹂닿퀬 ??')),h('div',{className:'go-stat'},tt('gameOver.duration',{days:p.stats.day},'?댁쁺 湲곌컙: '+p.stats.day+'??)),h('div',{className:'go-stat'},tt('gameOver.stats',{c:p.stats.c,r:p.stats.r,t:p.stats.t,o:p.stats.o},'遊됱뇙: '+p.stats.c+' | ?먯썝: '+p.stats.r+' | ?좊ː: '+p.stats.t+' | ?됯?: '+p.stats.o)),h('div',{className:'go-msg'},'"'+msg+'"'),h('div',{style:{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:'#33cccc',marginTop:12,letterSpacing:1}},'GRANT: ACTIVE ??RENEWAL AVAILABLE')),btns);
}
function Tutorial(p){
  var s1=useState(0),step=s1[0],setStep=s1[1];
  var koSteps=[
    {lines:["ORACLE ?몄궗 ?꾨줈?좎퐳??媛쒖떆?⑸땲??","","?섏쁺?⑸땲?? PILEHEAD.","","?뱀떊? ORACLE Proxy Network","?쒓뎅 吏遺??珥덈? 吏?섍??쇰줈 諛쒕졊?섏뿀?듬땲??","","[寃?대맖] ?? 誘몄???諛붿씠?ъ뒪 EV-誇媛 異쒗쁽?덉뒿?덈떎.","媛먯뿼泥대뒗 ?대?泥대줈 蹂?댄븯硫?","???멸퀎 二쇱슂 ?꾩떆媛 遊됱뇙 以묒엯?덈떎."],choices:[{label:"怨꾩냽",next:1}]},
    {lines:["[?꾨Т 釉뚮━??","","ORACLE? ?꾨줈硫뷀뀒?곗뒪瑜?,"?곷? ?몃젰?쇰줈 遺꾨쪟?섍퀬 ?덉뒿?덈떎.","","?뱀떊???꾨Т:","??遊됱뇙 援ъ뿭 愿由?諛??대?泥????,"??湲곗? ?댁쁺 珥앷큵","??ORACLE 吏???댄뻾 諛??몃? ?꾪삊 媛먯떆","","媛꾨?吏?4紐낆씠 ?뱀떊??蹂댁쥖?⑸땲??"],choices:[{label:"怨꾩냽",next:2}]},
    {lines:["4媛吏 ?듭떖 吏?쒕? 愿由ы빀?덈떎.","","{{icon-c}} 遊됱뇙 ??遊됱뇙???좎???,"{{icon-r}} ?먯썝 ???앸웾, ?섏빟?? ?λ퉬","{{icon-t}} ?좊ː ??湲곗? ?몄썝???좊ː??,"{{icon-o}} ?됯? ??ORACLE???뱀떊??????됯?","","?대뒓 吏?쒕뱺 0???섎㈃ ?꾨Т???ㅽ뙣?⑸땲??","","???쇱そ / ?ㅻⅨ履???,"移대뱶瑜?諛???좏깮?섏떗?쒖삤."],choices:[{label:"寃뚯엫 ?쒖옉",next:-1}]}
  ];
  ...
  var koHL=[['PILEHEAD','#f0a030'],['ORACLE',uiC],['EV-\u03A3','#33cccc'],['?대?泥?,'#33cccc'],['?꾨줈硫뷀뀒?곗뒪','#ff6644'],['[寃?대맖]','#ff4444'],['遊됱뇙??,uiC],['遊됱뇙',uiC],['?먯썝',uiC],['?좊ː??,uiC],['?좊ː',uiC],['?됯?',uiC]];
  var i18nHL=(typeof t==='function')?t('tutorial.highlights'):null;
  var enHL=(i18nHL&&Array.isArray(i18nHL))?i18nHL.map(function(w){var cm={'PILEHEAD':'#f0a030','ORACLE':uiC,'EV-誇':'#33cccc','PROMETHEUS':'#ff6644','[REDACTED]':'#ff4444','Containment':uiC,'Resources':uiC,'Trust':uiC,'Evaluation':uiC};return[w,cm[w]||uiC]}):null;
  var HL=enHL||koHL;
...
function RewardScreen(p){
  ...
  var miniBar=function(fx){return h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:4,marginTop:8}},
    ['c','r','t','o'].map(function(k){var cur=p.stats[k];var chg=(fx[k]||0)*5;var nxt=Math.max(0,Math.min(100,cur+chg));var isPos=chg>0;var isNeg=chg<0;
      return h('div',{key:k,style:{fontSize:9,fontFamily:"'Share Tech Mono',monospace"}},
        ...
        h('div',{style:{textAlign:'center',marginTop:1,color:isPos?'var(--ui)':isNeg?'rgba(255,141,97,.9)':'rgba(var(--ui-rgb),.3)',fontSize:8}},chg!==0?(isPos?'+':'')+chg:'쨌')
      )}))};
  return h('div',{className:'screen'},
    ...
    h('div',{style:{width:'100%',maxWidth:440,display:'flex',justifyContent:'center',gap:12,margin:'4px 0',flexShrink:0,flexWrap:'wrap'}},
      ['c','r','t','o'].map(function(k){var v=p.stats[k];var nm={c:tt('stats.c',null,'遊됱뇙'),r:tt('stats.r',null,'?먯썝'),t:tt('stats.t',null,'?좊ː'),o:tt('stats.o',null,'?됯?')};var d=v<=20;return h('span',{key:k,style:{fontFamily:"'Share Tech Mono',monospace",fontSize:11,color:d?'#ff4444':'rgba(var(--ui-rgb),.7)',letterSpacing:1}},nm[k]+':'+v)})),
    h('div',{style:{fontSize:13,color:'var(--ui-text)',textAlign:'center',margin:'4px 0 8px'}},tt('reward.pickCount',{count:count},count+'媛?以??좏깮')),
    dangerC&&h('div',{style:{background:'rgba(var(--ui-rgb),.05)',border:'1px solid rgba(var(--ui-rgb),.22)',borderRadius:3,padding:'8px 12px',margin:'0 0 8px',maxWidth:440,width:'100%',fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:'rgba(var(--ui-rgb),.8)',letterSpacing:.5,lineHeight:1.7,boxSizing:'border-box'}},
      h('div',{style:{color:'rgba(var(--ui-rgb),.4)',fontSize:9,marginBottom:4,letterSpacing:1}},tt('news.headlineAlert',null,'[ ORACLE // ?댁쁺 ?곹깭 寃쎈낫 ]')),
      h('div',null,tt('news.headlineWarn1',null,'KR-INIT-001 遊됱뇙 ?꾩쟾???꾧퀎 ?꾨떖 ?덉륫.')),
      h('div',null,tt('news.headlineWarn2',null,'?쒓뎅吏遺 ?덉젙??100% ???꾩떆 ?댁쁺 沅뚰븳 ?먮룞 留뚮즺 ?덉감 媛쒖떆.')),
      h('div',{style:{marginTop:5,color:'rgba(var(--ui-rgb),.4)',fontSize:9}},tt('news.headlineWarn3',null,'GRANT EXPIRED ?덉감 以鍮?以? ?좏깮???좎쓽?섏떗?쒖삤.'))
    ),
    h('div',{style:{flex:1,width:'100%',maxWidth:440,position:'relative',minHeight:0}},
      ...
      av.map(function(r,i){var fl=fxList(r.fx);var isSel=sel===i;var willEnd=p.stats.c+(r.fx.c||0)*5>=100;var rLoc=(typeof tc==='function')?tc('rewards',r.id,null):null;var rTitle=(rLoc&&rLoc.title)||r.title;var rDesc=(rLoc&&rLoc.desc)||r.desc;return h('div',{key:r.id,className:'oracle-card'+(isSel?' is-selected':''),onClick:function(){setSel(i)}},
        ...
        h('div',{className:'oracle-card__effects'},
          fl.pos.map(function(e){var arrow=Math.abs(e.v)>=10?'?꿎뼯':'??;return h('span',{key:e.k,className:'oracle-card__effect oracle-card__effect--pos'},arrow+' '+SN[e.k]+' +'+e.v)}),
          fl.neg.map(function(e){var arrow=Math.abs(e.v)>=10?'?쇄뼹':'??;return h('span',{key:e.k,className:'oracle-card__effect oracle-card__effect--neg'},arrow+' '+SN[e.k]+' '+e.v)})
        ),
        miniBar(r.fx),
