...
function FieldMission(p){
  ...
  useEffect(function(){
    if(mission&&mission.spec&&typeof SFX!=='undefined'&&SFX.playLoop){
      ...
      return function(){ try{ if(SFX.stopLoop)SFX.stopLoop('creature_amb'); }catch(e){} };
    }
  },[]);
  ...
  var body=isStart
    ? h('div',{className:'fm-brief-wrap'},
        (node&&node.scene&&typeof TacticalFeed!=='undefined')?h(TacticalFeed,{scene:node.scene,risk:(typeof tacticalMaxRisk==='function')?tacticalMaxRisk(node):'',fxOff:fxOff,bg:heroSrc}):fmHero(),
