...
function App(){
  ...
  return h('div',{className:'screen'},
    ...
    h('div',{className:'info-bar'},
      ...
      (function(){var unlocked=logs.filter(function(id){return id.indexOf('LOG-')===0&&id.indexOf('LOG-INTRO-')!==0}).length;return h('span',{className:'info-tag',style:{cursor:'pointer',padding:'5px 6px'},onClick:function(){setRet('game');setPhase('logs')}},'LOG '+unlocked+'/'+ORACLE_LOGS.length)})(),
      (typeof ARCHIVE_ENTRIES!=='undefined')&&(function(){var uc=ARCHIVE_ENTRIES.filter(function(e){try{return e.unlock(logs)}catch(err){return false}}).length;if(uc===0)return null;var nc=ARCHIVE_ENTRIES.filter(function(e){try{return e.unlock(logs)&&seenArchive.indexOf(e.id)<0}catch(err){return false}}).length;return h('span',{className:'info-tag',style:{cursor:'pointer',padding:'5px 6px',color:nc>0?'#f0a030':'rgba(var(--ui-rgb),.7)',borderColor:nc>0?'rgba(240,160,48,.5)':'rgba(var(--ui-rgb),.2)',display:'inline-flex',alignItems:'center',gap:4},onClick:function(){setRet('game');setPhase('archive')}},'ARCHIVE'+(nc>0?' '+nc+' @':''))})(),
      (function(){var fc=(facility.completed||[]).length,fa=(facility.approved||[]).length,fp=(facility.pending||[]).length;var total=fc+fa+fp;if(total===0)return null;return h('span',{className:'info-tag',style:{cursor:'pointer',padding:'5px 6px',color:'#4ae',borderColor:'rgba(74,170,238,.4)'},onClick:function(){setShowFacility(true)}},'시설 '+fc+'/'+total)})(),
      logs.indexOf('LOG-EV-UNLOCK')>=0&&(function(){var col=typeof getCollectedEvidence==='function'?getCollectedEvidence(logs).length:0;return h('span',{className:'info-tag',style:{cursor:'pointer',padding:'5px 6px',color:'var(--ui)',borderColor:'rgba(var(--ui-rgb),.4)'},onClick:function(){setShowEvidence(true)}},'증거 '+col)})(),
      h('span',{className:'info-tag',style:{cursor:'pointer',marginLeft:'auto',padding:'5px 6px'},onClick:function(){setShowSettings(true)}},'☰')),
