...
function App(){
  ...
  return h('div',{className:'screen'},
    ...
    h('div',{className:'info-bar'},
      ...
      (typeof ARCHIVE_ENTRIES!=='undefined')&&(function(){var uc=ARCHIVE_ENTRIES.filter(function(e){try{return e.unlock(logs)}catch(err){return false}}).length;var nc=ARCHIVE_ENTRIES.filter(function(e){try{return e.unlock(logs)&&seenArchive.indexOf(e.id)<0}catch(err){return false}}).length;return h('span',{className:'info-tag',style:{cursor:'pointer',padding:'6px 10px',color:nc>0?'#f0a030':'rgba(var(--ui-rgb),.7)',borderColor:nc>0?'rgba(240,160,48,.5)':'rgba(var(--ui-rgb),.2)',display:'inline-flex',alignItems:'center',gap:4},onClick:function(){setRet('game');setPhase('archive')}},'ARCHIVE '+uc,nc>0?h('span',{style:{display:'inline-block',width:5,height:5,borderRadius:'50%',background:'#f0a030',boxShadow:'0 0 4px rgba(240,160,48,.6)'}}):null)})(),
