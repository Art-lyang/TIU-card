...
function App(){
  ...
  var nextCard=function(s,g,lg,cq,curAct){var a=curAct||act;if(cq&&cq.length>0){setCurCard(cq[0]);setChainQueue(cq.slice(1))}else{var c=drawCard(s,g,lg,cooldowns,recentCards,a,transRoute,facility);setCurCard(c);setRecentCards(function(p){var n=p.concat([c.id]);return n.length>30?n.slice(n.length-30):n})}};
  ...
  var hDlg=function(c){SFX.play('dialogue');var ns=applyFx(stats,c.fx||{}),ng=gi+(c.g||0);ns.c=Math.max(5,Math.min(95,ns.c));ns.r=Math.max(5,Math.min(95,ns.r));ns.t=Math.max(5,Math.min(95,ns.t));ns.o=Math.max(5,Math.min(95,ns.o));setStats(ns);setGi(ng);if(curDlg&&c.trust!==undefined)modTrust(curDlg.char,c.trust);var di=curDlg?DIALOGUES.indexOf(curDlg):-1;var csi=curDlg?DIALOGUES.filter(function(d,i){return d.char===curDlg.char&&i<=di}).length-1:0;checkLogs(ns,ng,null,curDlg?curDlg.char:null,csi);Save.saveGame(ns,ng,act,actFlags,transRoute);
    // 간부진 자기소개 연속 재생 — intro 4명 다 끝날 때까지 카드 안 넘어감
  ...
  var startNewGamePlus=function(){
    ...
    var rl=logs.filter(function(id){return id.indexOf('LOG-INTRO-')!==0&&id!=='ONCE-CA-001'});
  ...
  return h('div',{className:'screen'},
    ...
    h('div',{className:'info-bar'},
      ...
      (typeof ARCHIVE_ENTRIES!=='undefined')&&(function(){var uc=ARCHIVE_ENTRIES.filter(function(e){try{return e.unlock(logs)}catch(err){return false}}).length;var nc=ARCHIVE_ENTRIES.filter(function(e){try{return e.unlock(logs)&&seenArchive.indexOf(e.id)<0}catch(err){return false}}).length;return h('span',{className:'info-tag',style:{cursor:'pointer',padding:'6px 10px',color:nc>0?'#f0a030':'rgba(157,255,116,.7)',borderColor:nc>0?'rgba(240,160,48,.5)':'rgba(145,255,106,.2)',display:'inline-flex',alignItems:'center',gap:4},onClick:function(){setRet('game');setPhase('archive')}},'ARCHIVE '+uc,nc>0?h('span',{style:{display:'inline-block',width:5,height:5,borderRadius:'50%',background:'#f0a030',boxShadow:'0 0 4px rgba(240,160,48,.6)'}}):null)})(),
      ...
      logs.indexOf('LOG-EV-UNLOCK')>=0&&(function(){var col=typeof getCollectedEvidence==='function'?getCollectedEvidence(logs).length:0;return h('span',{className:'info-tag',style:{cursor:'pointer',padding:'6px 10px',color:'#9dff74',borderColor:'rgba(145,255,106,.4)'},onClick:function(){setShowEvidence(true)}},'증거 '+col)})(),
