...
function App(){
  ...
  if(phase==='briefing')return h(BriefingScreen,{act:act,stats:stats,transRoute:transRoute,onEnter:function(){
      Save.saveGame(stats,gi,act,actFlags,transRoute);var go=chkGameOver(stats);if(go){SFX.play('gameover');doGO(go,stats,gi);return}nextCard(stats,gi,logs,chainQueue);setPhase('game')}});
