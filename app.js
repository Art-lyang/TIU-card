...
function App(){
  ...
  var persistGame=function(s,g,a,af,tr,cd,rc,curCt,cq,fac){
    Save.saveGame(
      s,g,a,af,tr,
      cd||cooldowns,
      rc||recentCards,
  ...
  useEffect(function(){
    ...
    if(sg&&sg.act){setAct(sg.act);if(sg.actFlags)setActFlags(sg.actFlags);if(typeof sg.transRoute==='string')setTransRoute(sg.transRoute);if(sg.cooldowns)setCooldowns(sg.cooldowns);if(sg.recentCards)setRecentCards(sg.recentCards);if(Array.isArray(sg.chainQueue))setChainQueue(sg.chainQueue);if(typeof sg.ct==='number')setCt(sg.ct)}
    ...
    loadActiveSpecs();
    setCurCard(drawCard(initStats,initGi,sl||['LOG-001'],initCd,initRecent,initAct,initRoute, sf||{approved:[],pending:[],completed:[],proposed:[]}));
  },[]);
  ...
  var hEvening=function(){var liveLogs=getLiveLogs(logs);var go=chkGameOver(stats);if(go){SFX.play('gameover');doGO(go,stats,gi);return}
    ...
    if(stats.day>35){var teid=resolveTimeUp(stats,gi,trust,liveLogs);SFX.play('gameover');doGO('\uc138\uc158 \ub9cc\ub8cc',stats,gi,teid);return}
  ...
  var loadSnapshot=function(slot){
    ...
    setLogs(plogs);if(typeof window!=='undefined')window.__ts_liveLogs=plogs.slice();setTrust(ptrust);setUsedDlg(pud);setUsedEvening(pue);setFacility(pfac);
    setCt(typeof pg.ct==='number'?pg.ct:0);setCurDlg(null);setCurMission(null);setPendingBonus(null);
  ...
  return h('div',{className:'screen'},
    ...
    h('div',{className:'info-bar'},
      ...
      h('span',{className:'info-tag'},'MIS '+(ct+1)+'/'+cpd),
      (function(){var unlocked=logs.filter(function(id){return id.indexOf('LOG-')===0&&id.indexOf('LOG-INTRO-')!==0}).length;return h('span',{className:'info-tag',style:{cursor:'pointer'},onClick:function(){setRet('game');setPhase('logs')}},'LOG '+unlocked+'/'+ORACLE_LOGS.length)})(),
      ...
      (function(){var fc=(facility.completed||[]).length,fa=(facility.approved||[]).length,fp=(facility.pending||[]).length;var total=fc+fa+fp;if(total===0)return null;return h('span',{className:'info-tag',style:{cursor:'pointer',color:'#4ae',borderColor:'rgba(74,170,238,.4)'},onClick:function(){setShowFacility(true)}},'시설 '+fc+'/'+total)})(),
      logs.indexOf('LOG-EV-UNLOCK')>=0&&(function(){var col=typeof getCollectedEvidence==='function'?getCollectedEvidence(logs).length:0;return h('span',{className:'info-tag',style:{cursor:'pointer',color:'var(--ui)',borderColor:'rgba(var(--ui-rgb),.4)'},onClick:function(){setShowEvidence(true)}},'증거 '+col)})(),
      h('span',{className:'info-tag',style:{cursor:'pointer',marginLeft:'auto'},onClick:function(){setShowSettings(true)}},'☰')),
    h(CardC,{card:curCard,onSwipe:swipe,onPreview:setPreview,gi:gi,day:stats.day,onOracleBlock:function(msg){setToastType('oracle');setToast(msg);setTimeout(function(){setToast('')},2600)},onReply:function(msg){setToastType('');setToast(msg);setTimeout(function(){setToast('')},1500)}}),
