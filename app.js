...
function App(){
  ...
  if(phase==='go'){
    ...
    return withOracleLink(h(GameOver,{stats:stats,reason:gor,gi:gi,sessions:sessions,endNarr:endNarr,endId:endId,endImg:endImg,resultDay:goDay,onRestart:goBack||restart,onMainMenu:goBack||returnToMainMenu,onLogs:function(){setRet('go');setPhase('logs')},onArchive:function(){setRet('go');setPhase('archive')},onEndings:function(){setRet('go');setPhase('endings')},onAchievements:function(){setRet('go');setPhase('achievements')}}));
  }
  ...
  return withOracleLink(h('div',{className:'screen'},
    ...
    glitchLevel===3&&fxMode!=='off'&&h(GlitchOverlay,{level:3,fxMode:fxMode,onComplete:function(){setGlitchLevel(0)}}),DEV&&h(DevMissionLauncher,{open:showDevPanel,onToggle:function(){setShowDevPanel(function(v){return !v})},onLaunch:launchDebugMission,onLaunchBriefing:launchDebugBriefing,onLaunchEnding:launchDebugEnding,onLaunchSting:launchDebugSting,onLaunchTriggerCard:launchDebugTriggerCard,onPreviewPanel:function(which){setDevPanel(which)},onPreviewMini:function(){try{var _mids=Object.keys((typeof FIELD_MINIGAME_LIBRARY!=='undefined'&&FIELD_MINIGAME_LIBRARY)||{});localStorage.setItem('ts_minigamesSeen',JSON.stringify(_mids));}catch(e){}setRet(phase);setPhase('miniguide');},onPreviewCctv:previewCctv,onPreviewOracleOv:previewOracleOv}),DEV&&devPreview()));
}
