...
function DevMissionLauncher(p){
  ...
  var panel=h('div',{style:{position:'fixed',left:10,bottom:46,zIndex:99999,width:330,maxHeight:'72vh',overflowY:'auto',padding:'10px 12px',background:'rgba(6,14,13,.97)',border:'1px solid rgba(57,217,138,.5)',borderRadius:5,boxShadow:'0 6px 24px rgba(0,0,0,.6)'}},
    ...
    p.onLaunchSting?lbl('▓ CCTV 트리거 스팅 (조우 직전)','#ff8c2b'):null,
    p.onLaunchSting?h('button',{key:'sting-bs',onClick:function(){p.onLaunchSting('brainseeker')},style:{display:'block',width:'100%',textAlign:'left',font:'11px monospace',padding:'7px 9px',margin:'3px 0',cursor:'pointer',background:'rgba(22,14,6,.9)',color:'#ff8c2b',border:'1px solid rgba(255,140,43,.45)',borderRadius:3}},h('span',{style:{opacity:.7,marginRight:8}},'[CCTV]'),'브레인시커 → M-010'):null,
...
function App(){
  ...
  var launchDebugSting=function(key){var mid='M-010';if(typeof MISSIONS==='undefined'||!MISSIONS[mid])return;debugMissionRef.current=true;setShowDevPanel(false);setCurMission(mid);setCctvSting(key||'brainseeker')};
  ...
  if(phase==='go'){
    ...
    return withOracleLink(h(GameOver,{stats:stats,reason:gor,gi:gi,sessions:sessions,endNarr:endNarr,endId:endId,endImg:endImg,resultDay:goDay,onRestart:goBack||restart,onMainMenu:goBack||returnToMainMenu,onLogs:function(){setRet('go');setPhase('logs')},onArchive:function(){setRet('go');setPhase('archive')},onEndings:function(){setRet('go');setPhase('endings')}}));
  }
